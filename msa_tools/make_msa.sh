...
then
    PIPE_DIR="/databases"
    echo "############################################################################################"
    echo "###   WARNING!!! YOU ARE RUNNING HHBLITS ON A NON-DATABASE NODE. IS THIS INTENTIONAL???  ###"
    echo "############################################################################################"
    exit
fi
...
# /net/software/signalp/bin/signalp --fastafile $in_fasta --organism other --output_dir $tmp_dir --format none --mode slow
