...
def remove_duplicate_sequences(
    ...
    for i, seq in enumerate(sequences):
        ...
        if seq not in seen:
            ...
            # Get max bond deviation (unconstrained_only preferred)
            def get_max_bond_dev(m: Optional[Dict]) -> float:
                if m is None:
                    return float('inf')
                # Try to get unconstrained_only first
                bond_geom = m.get("bond_length_geometry", {})
                if "unconstrained_only" in bond_geom:
                    return bond_geom["unconstrained_only"].get("max_deviation", float('inf'))
                elif "all" in bond_geom:
                    return bond_geom["all"].get("max_deviation", float('inf'))
                elif "max_deviation" in bond_geom:
                    return bond_geom["max_deviation"]
                return m.get("max_bond_dev", float('inf'))

            existing_geom = get_max_bond_dev(existing_met)
            new_geom = get_max_bond_dev(met)

            if new_geom < existing_geom:
                seen[seq] = (pdb, met, i)
                LOGGER.debug(f"Replacing duplicate seq (idx {seen[seq][2]} -> {i}): "
                            f"bond_dev {existing_geom:.4f} -> {new_geom:.4f}")
