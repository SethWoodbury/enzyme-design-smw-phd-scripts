...
- **Bond Geometry Minimization**: Optional `minimize_bond_angles`/`minimize_bond_lengths` for better idealization (enabled by default)
...
```bash
/software/containers/pyrosetta.sif python constrained_cart_relax.py --step01_json ... --params ...
```
...
1. Starting with moderate weight (default: 0.8)
...
These provide finer control over geometry idealization and are **enabled by default**.
To disable: `--disable_bond_geometry_min`
...
- **Total internal rounds** = M × N (default: 3 × 5 = 15)
...
| `--fastrelax_ramp_stages` | 5 | Number of ramping stages (N). Total = M × N |
| `--cart_bonded_weight` | 0.8 | Initial cart_bonded weight |
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `--scorefunction` | ref2015_cart | Scorefunction to use. Options: `ref2015_cart`, `beta_jan25_cart`, `beta_nov16_cart`. beta_jan25_cart is the latest and may give better results. |
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `--enable_bond_geometry_min` | True | Enable `minimize_bond_angles` + `minimize_bond_lengths` in FastRelax |
...
| `balanced` | 3×5=15 | `enable_bond_geometry_min=True`, `max_adaptive_rounds=10` (default settings) |
| `thorough` | 5×5=25 | `bond_length_tolerance=0.03`, `bond_angle_tolerance=5.0`, `max_adaptive_rounds=15` |
| `aggressive` | 3×5=15 | `cart_bonded_weight=1.2`, `cart_bonded_max=4.0`, `cart_bonded_scale_factor=2.0`, `max_expansions=5` |
...
```json
{
  ...
  "convergence": {
    ...
    "total_internal_rounds_per_cycle": 15,
...
3. Ensure `--enable_bond_geometry_min` is on (default)
...
- **cart_bonded_weight=2.0** provides optimal bond geometry (default updated from 0.8)
- **ref2015_cart** outperforms beta_jan25_cart for bond geometry
- **1×3 to 3×5 cycles** perform similarly when cart_bonded is high
