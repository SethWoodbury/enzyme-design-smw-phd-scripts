...
def get_pocket_residues(pose):
    """
    Identify residues in the pocket based on proximity to ligand atoms.
    """
...
def get_residue_rmsd(residue1, residue2, specified_atoms=None):
    """
    Calculate RMSD between specified atoms of two residues.
    """
...
def calculate_pnear(scores, rmsds, lambda_val=1.5, kbt=0.62):
    """
    Calculate PNear value given scores, RMSDs, lambda, and kbt.
    """
    nscores = len(scores)
...
def filter_scores(scores, filters):
    """
    Apply filters to scores and return the filtered results.
    """
    ...
    for s in filters.keys():
        if filters[s] is not None and s in scores.keys():
            ...
            print(f"{s:<24} {filters[s][1]:<2} {val:>7.3f}: {len(filtered_scores)} designs left. {n_passed} pass ({(n_passed/len(scores))*100:.0f}%).")
    return filtered_scores

def plot_scores(scores):
    """
    Plot ChemNet scores vs. lddt.
    """
...
# Initialize PyRosetta with specified params
extra_res_fa = ""
...
# Handle scorefile and pdb paths
...
# Assign input PDB's to design names
ref_pdbs = {}
for d in designs:
    for p in args.pdb:
...
# Plot scores if less than 10 designs
if len(scores) < 10:
    plot_scores(scores)

# Define chi pairs (used in some analysis, not shown in provided code)
...
def process(q):
    while True:
        ...
        # Loading models
        chemnet_pdb_file = os.path.join(args.chemnet_pdb_path, d + "_model.pdb")
        print(f"Loading ChemNet PDB file: {chemnet_pdb_file}")
        pdbstr = open(chemnet_pdb_file, "r").read()
        models = []
        ...
        for jj, cr in enumerate(catalytic_residues[d]):
            ...
            for j, p in enumerate(model_poses):
                try:
                    ...
                    print(f"Warning: {cr} not found in pocket_residues_pdb[{d}]")
                    continue
            ...
            if cr in catalytic_residue_rmsds[d].columns:
                ...
                print(f"Warning: Column {cr} not found in catalytic_residue_rmsds[{d}]")

            if cr in model_res_scores[d].columns:
                ...
                print(f"Warning: Column {cr} not found in model_res_scores[{d}]")
...
pool = multiprocessing.Pool(processes=args.nproc, initializer=process, initargs=(the_queue,))
