...
def process(q):
    while True:
        ...
        DF = pd.DataFrame()
        _df = scores.loc[scores.label.str.contains(d)]
        # _df_bond = scores_bond.loc[scores_bond.label.str.contains(d)]
    
        # print(d, len(_df.loc[_df.kabsch < 1.5])/len(_df))
        ...
        ## Loading models
        #pdbstr = open(d + "_model.pdb", "r").read()
        ...
        ## Making a reference pose where heme is flipped
        # Adjusting the propionate chis such that they match at least a little bit
        ref_lig_seqpos = None
        ...
        ## Calculating rmsd of heme in ChemNet models against normal and flipped heme
        ...
        for idx, row in _df.iterrows():
            ...
            rmsd = pyrosetta.rosetta.core.scoring.automorphic_rmsd(ref_pose.residue(ref_lig_seqpos),
                                                                           p.residue(mdl_lig_seqpos), False)
            _df.at[idx, "rmsd_new"] = rmsd

        # for sct in scoreterms:
        #     DF.at[i, f"good_{sct}"] = _df.loc[_df.kabsch < 1.5][sct].mean()

        ## plddt and rmsd based on sorting by plddt (1D track information)
        DF.at[i, "rmsd_top5"] = 9.9
        DF.at[i, "plddt_top5"] = 0.0
        # DF.at[i, "lddt_pnear_good"] = 0.0
        # DF.at[i, "plddt_pnear_good"] = 0.0
        ...
        # DF.at[i, "lddt_pnear_good"] = calculate_pnear(np.array(_df.loc[_df.kabsch < 1.5]["lddt"]), np.array(_df.loc[_df.kabsch < 1.5]["rmsd_new"]), lambda_val=1.5, kbt=0.62 )
        # DF.at[i, "plddt_pnear_good"] = calculate_pnear(np.array(_df.loc[_df.kabsch < 1.5]["plddt"]), np.array(_df.loc[_df.kabsch < 1.5]["rmsd_new"]), lambda_val=1.5, kbt=0.62 )
        ...
        DF.at[i, "rmsd_pde_top5"] = 9.9
        DF.at[i, "plddt_pde_top5"] = 0.0
        if DF.at[i, "frac_good_kabsch"] > 0.0:
        ...
        for jj, cr in enumerate(catalytic_residues[d]):
            ...
            DF.at[i, f"rmsd_std_catres{jj}"] = np.std(catalytic_residue_rmsds[d][cr])
             #DF.at[i, f"rmsd_good_catres{jj}"] = np.median(good_rmsds)
            # DF.at[i, f"rmsd_std_good_catres{jj}"] = np.std(good_rmsds)
    
            good_res_scores = model_res_scores[d][cr][_df_good.model_idx.values.tolist()]
            DF.at[i, f"u_catres{jj}"] = np.average(model_res_scores[d][cr])
            DF.at[i, f"u_std_catres{jj}"] = np.std(model_res_scores[d][cr])
            # DF.at[i, f"u_good_catres{jj}"] = np.average(good_res_scores)
            # DF.at[i, f"u_std_good_catres{jj}"] = np.std(good_res_scores)
        ...
        # DF.at[i, "rmsd_pocket_good"] = pocket_residue_rmsds_good[d][pocket_residues_pdb[d]].median().median()
        ...
        # DF.at[i, "rmsd_pocket_good_worst10"] = np.median(sorted(pocket_residue_rmsds_good[d][pocket_residues_pdb[d]].median())[-10:])
        ...
        ## Dumping top 5 chemnet models based on plddt as full protein PDB's
            ...
            for j, (idx, row) in enumerate(_df.sort_values("plddt", ascending=False).iloc[:5].iterrows()):
                ...
                for rn in range(1, _p.size()+1):
                    ...
                    if res.name3() == "HIS" and res.name() != ref_pose.residue(orig_resno).name():
                        print(f"Fixing the HIS tautomer of residue {res.name()}-{rn} with {ref_pose.residue(orig_resno).name()}-{orig_resno}")
                        ...
                        mutres.apply(_p)
                        res = _p.residue(rn)
                    ## Need to make sure that residues that are terminal in chemnet cut-out,
                    # but are not terminal in the actual pose do not interfere with anything
                    skip_terminal_atoms = False
                    if res.natoms() != pose2.residue(orig_resno).natoms():
                        skip_terminal_atoms = True
                ...
                #pose2.dump_pdb(f"top5_models/{d}_chemnet_{j}.pdb")
...
### Filtering chemnet scores
# interesting_scores = {
#     'frac_good_kabsch': [0.3, '>='],
#     'good_plddt': [0.65, '>='],
#     'good_rmsd': [2.0, '<='],
#     'rmsd_good_catres0': [1.0, '<='],
#     'rmsd_std_good_catres0': [0.5, '<='],
#     'u_good_catres0': [1.0, '<='],
#     'u_std_good_catres0': [0.3, '<='],
#     'u_pocket': [0.5, '<='],
#     'u_pocket_std': [0.1, '<='],
#     'u_pocket_worst': [2.0, '<=']}

# filtered_scores = filter_scores(DF, interesting_scores)
...
#     os.makedirs(f"{args.outdir}/good", exist_ok=True)
#     for idx, row in filtered_scores.iterrows():
#         copy2(ref_pdbs[row.description], f"{args.outdir}/good/{row.description}.pdb")
# else:
#     print("No good designs found, that's a bummer...\n"
#           "Maybe adjust filters?")
