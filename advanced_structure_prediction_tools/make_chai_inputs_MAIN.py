...
if __name__ == "__main__":
  ...
  #  parser.add_argument("--ligand_smiles", type=str,
   #                     help="JSON string defining one ligand SMILES pair (e.g. '[\"[Zn+2]\",\"CC(...)\"]'). "
    ...
    parser.add_argument("--pdb_prefix", type=str, nargs="+", default=None,
    ...
    parser.add_argument("--append_commands", action="store_true", default=False,
                        help="Append to the command file instead of overwriting. "
                             "Useful for multi-group runs with different settings per group.")
  ...
  #      global_smiles = json.loads(args.ligand_smiles)
   #     if not isinstance(global_smiles, list) or len(global_smiles) != 2:
    ...
    if args.pdb_path:
        ...
        if args.pdb_prefix:
            before_count = len(pdb_files)
            pdb_files = [
                f for f in pdb_files
                if any(os.path.splitext(os.path.basename(f))[0].startswith(pfx)
                       for pfx in args.pdb_prefix)
            ]
            print(f"Prefix filter ({', '.join(args.pdb_prefix)}): "
                  f"{len(pdb_files)} of {before_count} PDB files matched")
    ...
    cmd_count = 0
    open_mode = "a" if args.append_commands else "w"
    with open(commands_file, open_mode) as cf:
