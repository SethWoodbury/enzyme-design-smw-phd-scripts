...
def extract_ca_coords(
    ...
    When n_ignore or c_ignore are specified, the first n_ignore and last c_ignore
    protein residues (standard and modified amino acids) are skipped.
    Ligands/HETATM (non-amino-acid) are never skipped.
    ...
    for model in structure:
        for chain in model:
            ...
            if c_ignore > 0:
                effective_residues = protein_residues[n_ignore:len(protein_residues) - c_ignore]
            else:
                effective_residues = protein_residues[n_ignore:]
...
def extract_coords_res(
    ...
    When n_ignore/c_ignore are specified, residue_num is treated as an "effective"
    index (1-based) after skipping the first n_ignore and last c_ignore protein
    residues on the specified chain.
...
def get_resname_by_chain_resnum(
    ...
    When n_ignore/c_ignore are specified, resnum is treated as an effective
    index (1-based) after skipping terminal residues.

    Args:
        structure: Biopython Structure object
        chain_id: Chain identifier
        resnum: Effective residue index (1-based) when using terminal ignoring
        n_ignore: Number of protein residues at N-terminus to ignore (default: 0)
        c_ignore: Number of protein residues at C-terminus to ignore (default: 0)
...
def filter_biotite_protein_residues(
    ...
    """
    Filter Biotite AtomArray to protein residues with terminal ignoring.

    Args:
        atoms: Biotite AtomArray
        n_ignore: Number of residues to ignore at N-terminus
        c_ignore: Number of residues to ignore at C-terminus
    ...
    # Get unique residue IDs (chain + res_id combinations)
    # Create a combined identifier for each atom
    res_ids = protein_atoms.res_id
    unique_res_ids = np.unique(res_ids)

    # Apply terminal ignoring
    if c_ignore > 0:
        keep_res_ids = unique_res_ids[n_ignore:len(unique_res_ids) - c_ignore]
    else:
        keep_res_ids = unique_res_ids[n_ignore:]

    # Filter to kept residues
    keep_mask = np.isin(protein_atoms.res_id, keep_res_ids)
...
def calculate_tm_score(
    ...
    # Filter to protein with terminal ignoring
    af3_protein = filter_biotite_protein_residues(af3_atoms, n_ignore, c_ignore)
...
def calculate_catres_lddt(
    ...
    # Get effective residue IDs for mapping
    af3_res_ids = np.unique(af3_protein.res_id)
    ref_res_ids = np.unique(ref_protein.res_id)
    ...
    for catres in catres_list:
        ...
        # Map effective index to actual residue ID
        if eff_idx < 1 or eff_idx > len(af3_res_ids) or eff_idx > len(ref_res_ids):
            vlog(verbose, f"  Catres {eff_idx}: Out of range, skipping", "WARN")
            continue

        af3_target_res_id = af3_res_ids[eff_idx - 1]
        ref_target_res_id = ref_res_ids[eff_idx - 1]

        # Get atoms for this residue (heavy atoms only)
        af3_res_mask = (af3_protein.res_id == af3_target_res_id) & (af3_protein.element != "H")
        ref_res_mask = (ref_protein.res_id == ref_target_res_id) & (ref_protein.element != "H")
        ...
        if len(af3_res_atoms) == 0 or len(ref_res_atoms) == 0:
            vlog(verbose, f"  Catres {eff_idx}: No atoms found, skipping", "WARN")
            continue
...
def get_plddt_per_residue_dic(
    ...
    - Protein residues use effective indexing (first n_ignore and last c_ignore skipped)
    - Ligand/HETATM residues always use raw residue numbers (never ignored)
    ...
    for ch_id, res_list in chain_protein_resids.items():
        if c_ignore > 0:
            real_subset = res_list[n_ignore:len(res_list) - c_ignore]
        else:
            real_subset = res_list[n_ignore:]
...
def calculate_rmsd_after_alignment(
    ...
    Args:
        ...
        n_ignore: Number of protein residues at N-terminus to ignore in structure1 (default: 0)
        c_ignore: Number of protein residues at C-terminus to ignore in structure1 (default: 0)
    ...
    # Extract CA coordinates for alignment (with terminal ignoring for structure1)
    ...
    for catres in catres_list:
        ...
        # AF3 structure uses effective indexing; reference uses full structure
        catres_coords1, catres_atom_name1 = extract_coords_res(
            structure1, ch_id, eff_idx, n_ignore, c_ignore
        )
        ...
        af3_resname = get_resname_by_chain_resnum(structure1, ch_id, eff_idx, n_ignore, c_ignore)
        ...
        af3_raw_resseq = get_raw_resseq_for_effective_index(structure1, ch_id, eff_idx, n_ignore, c_ignore)
...
def calculate_rmsd_after_tmalign(
    ...
    Args:
        ...
        ligand_groups_json: List of ligand group definitions
        verbose: Whether to print detailed progress
        n_ignore: Number of protein residues at N-terminus to ignore
        c_ignore: Number of protein residues at C-terminus to ignore
    ...
    try:
        ...
        # Get residue IDs for mapping
        af3_res_ids = np.unique(aligned_af3.res_id)
        ref_res_ids = np.unique(ref_protein.res_id)
        ...
        for catres in catres_list:
            ...
            # Get heavy atoms for this residue from aligned structures
            af3_res_mask = (aligned_af3.res_id == af3_target_res_id) & (aligned_af3.element != "H")
            ref_res_mask = (ref_protein.res_id == ref_target_res_id) & (ref_protein.element != "H")
...
def process_single_af3_pdb(
    ...
    ca_rmsd_tmalign, catres_rmsds_tmalign, ligand_rmsds_tmalign = calculate_rmsd_after_tmalign(
        af3_pdb, ref_pdb_path, catres_list, ligand_groups_json, verbose, n_ignore, c_ignore
    )
    ...
    plddt_per_residue_dic = get_plddt_per_residue_dic(af3_pdb, n_ignore, c_ignore)
    cat_res_plddts = []
    for i, catres in enumerate(catres_list):
...
def main() -> None:
    ...
    if n_ignore > 0 or c_ignore > 0:
        log(f"  Terminal ignoring: N-term={n_ignore} residues, C-term={c_ignore} residues")
    if catres_subset_blocks is not None:
