...
For each AF3 job directory, this script:
  1. Reads the canonical job name from `*_data.json` and renames the directory.
  2. Flattens the per-sample model files up to the job root, renaming them
     `<job>_idx_<N>_{model.cif, confidences.json, summary_confidences.json}`.
  3. Converts every indexed `*_model.cif` to `*_model.pdb`.
  4. Deletes each original CIF only after its PDB is verified on disk, plus
     any now-empty intermediate directories.

Two AF3 output layouts are handled:
  - Legacy: per-model subdirs `..._seed-1_sample-<N>/` containing
    `model.cif`, `confidences.json`, `summary_confidences.json`.
  - New:    a `samples/` dir holding `..._seed-<X>_sample-<Y>_{model.cif,
    confidences.json, summary_confidences.json}` files.

`--robust-mode` resumes jobs where flattening already happened (i.e.
`*_idx_<N>_*` files exist at the root) but conversion was interrupted.
"""
import os
import re
import glob
...
# Default converter. Invoked directly as an executable, so the path must point
# at something runnable (a wrapper script around maxit, not a bare .sif image).
DEFAULT_CIF_SIF = "/software/containers/users/sklein89/maxit.sif"

# Legacy AF3 layout: per-model subdirs named "...seed-1_sample-<N>".
# Note: only seed 1 is matched; multi-seed legacy jobs would need a wider regex.
MODEL_DIR_PATTERN = re.compile(r"seed-1_sample-(\d+)$")

# New AF3 layout: files inside "<job>/samples/" tagged with "_seed-<X>_sample-<Y>_".
...
# Already-flattened triplets at the job root:
# "<job>_idx_<N>_{model.cif, confidences.json, summary_confidences.json}".
...
def detect_existing_idx_triplets(job_root: Path, verbose=False) -> list[dict]:
    """Scan `job_root` for already-flattened `*_idx_<N>_*` triplets.

    Returns a list of info dicts shaped like `flatten_model` / `flatten_new_layout`
    output so the rest of the pipeline (especially `cleanup_after_verification`)
    can consume them uniformly:

        {"idx": "<N>", "moved": [paths that exist], "missing": [...], "model_dir": None}

    `model_dir` is always None here so cleanup never tries to delete anything
    ...
    for idx, srcs in ordered:
        ...
        info_list.append({
            ...
            "moved": have,
            "missing": missing,
            # Must be None: an earlier version set this to job_root, which would
            # let cleanup attempt to delete the root itself.
            "model_dir": None,
        })
...
def remove_empty_samples_dir(job_root: Path, verbose=False):
    """Delete `<job_root>/samples/` if it exists and is empty after flattening."""
...
def is_new_layout(job_root: Path) -> bool:
    """True if `<job_root>/samples/` holds any AF3 new-layout files."""
    samples = job_root / "samples"
...
def flatten_new_layout(job_root: Path, corrected_name: str, verbose=False) -> list[dict]:
    """Flatten the new AF3 layout up to the job root.

    Reads from `<job_root>/samples/`, groups files by their `_seed-X_sample-Y_`
    token, sorts the tokens alphanumerically, and moves each triplet to
    `<corrected_name>_idx_<i>_{confidences.json, model.cif, summary_confidences.json}`.

    Returns a list of info dicts mirroring `flatten_model`'s shape so
    `cleanup_after_verification` can consume both uniformly.
    """
    ...
    for i, token in enumerate(ordered_tokens):
        ...
        # `model_dir` is the shared samples/ dir (no per-model subdirs in the
        # new layout). Cleanup's `_is_safe_model_dir` guard rejects it because
        # the name doesn't match MODEL_DIR_PATTERN, so it's never deleted here;
        # `remove_empty_samples_dir` handles it after cleanup instead.
        flattened_info.append({
            "idx": str(i),
            "moved": moved,
            "missing": missing,
            "model_dir": samples_dir,
        })
...
def safe_remove(path: Path, verbose=False, job_root: Path=None):
    """Remove a file or directory; refuses to delete `job_root` itself.

    Failures are logged as warnings rather than raised so cleanup can continue.
    """
...
def parse_job_name(job_dir: Path) -> str:
    """Read the canonical job name from the single `*_data.json` file in `job_dir`."""
...
def rename_job_dir(job_dir: Path, corrected_name: str, verbose=False) -> Path:
    """Rename `job_dir` to its canonical name (sibling) and return the new path."""
...
def find_model_subdirs(job_root: Path) -> list[Path]:
    """Return legacy-layout per-model subdirs in `job_root`, sorted by sample index."""
...
def flatten_model(d: Path, corrected_name: str, job_root: Path, verbose=False) -> dict:
    """Move a legacy model subdir's three files up to `job_root` with `_idx_<N>_` names.

    Returns an info dict shaped like `flatten_new_layout`'s entries so cleanup
...
def convert_cifs_to_pdbs(job_root: Path, cif_sif: str, verbose=False, include_nonindexed=False) -> list[tuple[Path, Path]]:
    """Convert CIFs in `job_root` to PDBs using `cif_sif`.

    By default only `*_idx_<N>_model.cif` files are converted; with
    `include_nonindexed=True`, every `*.cif` at the job root is converted.
    Returns the list of `(cif, pdb)` pairs for which the PDB was produced.
    The CIFs themselves are left in place — `cleanup_after_verification` only
    removes them after re-confirming the PDB exists.
    """
...
def cleanup_after_verification(job_root: Path,
                               ...
                               verbose=False):
    """Delete intermediates only after verifying the intended outputs exist.

    Steps, in order:
      1. Remove each original CIF whose PDB is on disk (per-file check).
      2. Remove per-model subdirs that fully flattened (never the job root).
      3. Remove unrelated top-level files. Whitelist: flattened JSONs,
         produced PDBs, any pre-existing `*_idx_*` JSONs at the root, and
         `*_data.json`. Directories are never deleted in this step.
    """
...
def process_one(index: int,
                ...
                robust_mode: bool):
    """Process a single AF3 job dir end-to-end: rename → flatten → convert → cleanup.

    Branches on the detected layout (new vs. legacy). With `robust_mode`, also
    handles the case where flattening already happened on a previous run but
    conversion was interrupted (i.e. `*_idx_*` files exist at the root).
    """
    ...
    if is_new_layout(job_root):
        ...
        # Must return: the legacy-layout code below would otherwise re-process
        # this job and warn that no model subdirs were found.
        return
...
def worker(queue, cif_sif: str, disable_deletions: bool, verbose: bool, include_nonindexed_cifs: bool, robust_mode: bool):
    """Multiprocessing consumer: pull `(idx, subdir)` items from `queue` until None.

    Per-job exceptions are logged so one bad job doesn't kill the worker.
    """
    while True:
        item = queue.get()
        if item is None:
...
def main():
    ...
    if args.af3_dir:
        ...
        # One sentinel per worker so they each exit their queue loop.
        for _ in range(nwork):
            q.put(None)
        ...
        # Single-subdir mode: run inline, no worker pool.
        process_one(
            0,
