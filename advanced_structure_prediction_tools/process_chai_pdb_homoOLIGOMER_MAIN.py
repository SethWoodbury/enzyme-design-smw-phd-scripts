...
def extract_coords_res(structure, ref_res_num, n_ignore=0, c_ignore=0, n_oligomers=1):
    """
    Pull out all non-H atom coords + names for the catalytic residue
    numbered ref_res_num *in the reference*, across an n-mer.
    """
    # map to (chain_id, local_index)
    chain_id, local_i = map_ref_resnum_to_chain_and_local(
        ref_res_num, n_ignore, c_ignore, n_oligomers, structure
    )
    ...
    for model in structure:
        ...
        prot = [r for r in chain.get_residues() if r.id[0]==' ']
        eff_res = prot[n_ignore: len(prot)-c_ignore]
        target = eff_res[local_i-1]
        for atom in target:
...
def calculate_rmsd_after_alignment(structure1, structure2, catres_list, ligand_atom_mapping_list, n_ignore=0, c_ignore=0):
    ...
    for catres in catres_list:
        catres_coords1, catres_names1 = extract_coords_res(
            structure1, catres["res_num"], n_ignore, c_ignore, n_oligomers)
        catres_coords2, catres_names2 = extract_coords_res(
            structure2, catres["res_num"], 0, 0, n_oligomers)
...
def map_ref_resnum_to_chain_and_local(res_num, n_ignore, c_ignore, n_oligomers, structure):
    """
    Given a ref residue number in [1..total_effective_res * n_oligomers], 
    figure out which chain it lives on and what its index is *after*
    ignoring N/C terminal tags in that chain.
    Returns (chain_id, local_eff_index).
    """
    # 1) collect only chains that have protein ATOM residues
    # 1) collect only *protein* chains based on presence of a CA atom
    prot_chains = []
    for model in structure:
        for chain in model:
            # only consider it a protein chain if at least one ATOM‐record residue has a CA atom
            if any(residue.id[0] == ' ' and 'CA' in residue for residue in chain.get_residues()):
                prot_chains.append(chain)

    # --- INSERT THIS BLOCK ---
    # if you only found a single concatenated chain, replicate it so
    # len(prot_chains) == n_oligomers
    if len(prot_chains) == 1 and n_oligomers > 1:
        prot_chains = prot_chains * n_oligomers
    # now fall through to the usual check
    if len(prot_chains) != n_oligomers:
        raise ValueError(f"Expected {n_oligomers} chains but found {len(prot_chains)}")
    # --- END INSERTION ---

    if len(prot_chains) != n_oligomers:
        raise ValueError(f"Expected {n_oligomers} chains but found {len(prot_chains)}")
    # 2) how many residues *per* chain (before ignoring)?
    full = sum(1 for r in prot_chains[0].get_residues() if r.id[0]==' ')
    eff = full - n_ignore - c_ignore
    # 3) which chain?
    idx0 = res_num - 1
    chain_idx = idx0 // eff
    local = (idx0 % eff) + 1
    return prot_chains[chain_idx].id, local
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_oligomers", type=int, default=1,
                        help="Number of repeated subunits (chains) in your model (e.g. 3 for a trimer).")
    args = parser.parse_args()

    n_ignore = args.N_terminus_tag_length_to_ignore
    c_ignore = args.C_terminus_tag_length_to_ignore
    n_oligomers = args.n_oligomers
    ...
    for chai_pdb_num, chai_pdb in enumerate(chai_pdbs):
        ...
        for i, catres in enumerate(catres_list):
            chain_id, local_i = map_ref_resnum_to_chain_and_local(
                catres["res_num"], n_ignore, c_ignore, n_oligomers, ref_structure
            )
            key = (chain_id, local_i)
            val = plddt_dic.get(key, np.nan)
            updated_score_dic[f"{catres['name3']}{i+1}_plddt_idx_{chai_pdb_num}"] = val
            cat_res_plddts.append(val)
        # use np.nanmean to ignore any missing entries
        updated_score_dic[f"catres_plddt_idx_{chai_pdb_num}"] = np.nanmean(cat_res_plddts)
