    ...
    def process(q):
        while True:
            ...
            if args.trb is None:
                ...
                __trbfs_count = []
                for __trbf in __trbfs:
                    __trbf_bn = os.path.basename(__trbf)
                    __trbfs_count.append(max([i for i in range(len(__trbf_bn)) if pdbfile_bn.count(__trbf_bn[:i])]))
                trbfile = __trbfs[__trbfs_count.index(max(__trbfs_count))]
                print(f"[PDB]:[TRB] = [{pdbfile_bn}]:[{os.path.basename(trbfile)}]")
            ...
            # RFflow fixed positions will not be touched
            if TIP_ATOM:  # tip-atom RFflow TRB has a bug
            ...
            if args.only_seq is False:
                    ...
                    if any([n in _positions_to_not_design for n in rgn]):  # any fixed/motif residue in a contig region?
                        ...
                        for resno in rgn:
                            if resno not in _positions_to_not_design:
                                ...
                                # unless tip-atom RFflow was used. Then there's a bug...
                                _tmp_rgn.append(resno)
                                if resno == rgn[-1]:
                                    keep_regions_expanded_no_catres.append(_tmp_rgn)
                                # print(resno)
                ...
                if args.design_full is False:
                    if not TIP_ATOM:  # tip-atom RFflow TRB has a bug
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trb", nargs="+", type=str, help="TRB files from aa_RFflow")
    ...
    parser.add_argument("--ref_catres", type=str, nargs="+", help="Format: 'A15 Bxx'. Residue that is defined as catalytic in the RFflow input. This will be parsed from the trb con_ref_pdb_idx")
    parser.add_argument("--ref_designable", type=str, nargs="+", help="Format: 'A15 Bxx'. Residues that were motifs for RFflow, but can be redesigned during inpainting. This will be parsed from the trb con_ref_pdb_idx")
