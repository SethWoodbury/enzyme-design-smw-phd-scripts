...
def main():
    ...
    parser.add_argument("--pdb_path", help="Path including input PDBs.")
    ...
    parser.add_argument("--ref", nargs="+", default=None, type=str, help="Reference PDB(s) from diffusion")
    parser.add_argument("--ref_path", type=str, help="Path including reference PDB(s) from diffusion")
    ...
    if (args.ref == None and args.ref_path == None):
        raise ValueError
    if args.ref == None:
    ...
    def process(q, ref_poses):
        while True:
            ...
            if len(__rs) != 1 and len(args.ref) > 1:
                ...
                __rs_count = []
                for __r in __rs:
                    __r_bn = os.path.basename(__r)
                    __rs_count.append(max([i for i in range(len(__r_bn)) if pdbfile_bn.count(__r_bn[:i+1])]))
                ref_pdb = __rs[__rs_count.index(max(__rs_count))]

                print(f"[DESIGN]:[REF] = [{pdbfile_bn}]:[{os.path.basename(ref_pdb)}]")
            elif len(__rs) == 1:
                ref_pdb = __rs[0]
            ...
            if len(matched_residues) > 0:
                ...
                for cstno,r in enumerate(matched_residues.keys()):
                        ...
                        for i, res in enumerate(trb["con_ref_pdb_idx"]):    
                            if res == (matched_residues[r]["chain"], np.int64(r)):
                                ...
                                if tgt_resno_orig != 0 and pose2.residue(tgt_resno_orig).is_protein():
                                    for jj, tgtres in enumerate(trb["con_ref_pdb_idx"]):
                                        if tgtres == (matched_residues[r]["target_chain"], np.int64(tgt_resno_orig)):
                                            matched_residues_in_design[resno_in_design]["target_chain"] = trb["con_hal_pdb_idx"][jj][0]
                                            matched_residues_in_design[resno_in_design]["target_resno"] = trb["con_hal_pdb_idx"][jj][1]
                                            break
                                break
                    else:
                ...
                for i, ref_catres in enumerate(matched_residues.keys()):
                    if bb_inpaint is True:
                        try: tgt_catres = trb["con_hal_idx0"][trb["con_ref_idx0"].tolist().index(ref_catres-1)]+1
                        except: return
                    else:
