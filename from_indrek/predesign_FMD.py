...
if len(glob.glob(f"{pdbname}{suffix}_*.pdb")) > 0:
    print(f"{pdbname}{suffix}_* outputs already exist")
    sys.exit(0)
...
for i, p in enumerate(poses):
    ...
    scores_df.at[0, "description"] = f"{pdbname}{suffix}_{i}"
    scoring_utils.dump_scorefile(scores_df, scorefilename)
    p.dump_pdb(f"{pdbname}{suffix}_{i}.pdb")
