...
def add_matcher_line_to_pose(pose, ref_pose, tgt_residues, ref_residues):
    ...
    # for i, ref_resno in enumerate(ref_residues):
    #     tgt_resno = list(tgt_residues.keys())[i]
    #     _new_remarks.append(f"REMARK 666 MATCH TEMPLATE X {ligand_name}"
    #                         f"    0 MATCH MOTIF A {pose.residue(tgt_resno).name3()}  {tgt_resno}  {i+1}  1               \n")
...
parser.add_argument("--params", nargs="+", default=[f"{HOME}/home/ikalvet/projects/Heme/theozyme/HMM/HMM_high.params",
...
# native = pyr.pose_from_file(args.ref)

ref_poses = {}
# for ref_pdb in args.ref:
#     if any([os.path.basename(ref_pdb).replace(".pdb", "") in pdbf for pdbf in pdbfiles]):
#         ref_poses[ref_pdb] = pyr.pose_from_file(ref_pdb)
#         ref_path = os.path.dirname(ref_pdb)
...
def process(q):
    while True:
        ...
        # if "native" in pdbfile.split("_")[-4:]:
        #     ref_name = ref_path + "/" + "_".join(os.path.basename(pdbfile).split("_")[:-5]) + ".pdb"
        # else:
        #     ref_name = ref_path + "/" + "_".join(os.path.basename(pdbfile).split("_")[:-7]) + ".pdb"

        # print(os.path.basename(pdbfile), ref_name)
        ...
        #     print(f"{pdbfile}: does not contain CYS")
        #     continue
        # elif pose.sequence().count("C") > 1:
        #     print(f"{pdbfile}: multiple CYS found!")
        #     continue

        # cys_seqpos = pose.sequence().find("C") + 1
        ...
        # if ref_poses[ref_name].residue(cys_seqpos).name3() == "CYX":
        #     mutres = pyrosetta.rosetta.protocols.simple_moves.MutateResidue()
        #     mutres.set_res_name("CYX")
        #     mutres.set_target(cys_seqpos)
        #     mutres.apply(pose2)
