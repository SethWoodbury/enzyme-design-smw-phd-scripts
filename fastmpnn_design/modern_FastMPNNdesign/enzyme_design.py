...
class ConstraintManager:
    ...
    def add_constraints(self, pose, force=False):
        ...
        """
        if self._enzcst_failed and not force:
            print("  [Constraints] Skipping EnzConstraintIO: previous failure recorded")
            return

        mp_details = rosetta_utils.pose_has_mp_coordination(pose)
        if mp_details:
            print(f"  [Constraints] Skipping EnzConstraintIO: detected MP coordination patches ({len(mp_details)} residues)")
            for resno, name3, res_type in mp_details:
                print(f"  [Constraints]   MP residue: {name3}{resno} type={res_type}")
            self._enzcst_failed = True
            return

        # Only skip if enzyme constraints are already present. Coordinate constraints
        # (ligand/catres) should not block enzyme constraint addition.
        if pose.constraint_set().has_constraints() and not force:
            try:
                enzyme_types = {"AtomPairConstraint", "AngleConstraint", "DihedralConstraint"}
                existing = {c.__class__.__name__ for c in pose.constraint_set().get_all_constraints()}
                if existing & enzyme_types:
                    return
            except Exception:
                # If we cannot inspect existing constraints, fall through to add
                pass

        try:
            safe_pose = pose.clone()
            self._cst_io.add_constraints_to_pose(safe_pose, self._scorefxn, True)
            pose.assign(safe_pose)
            try:
                print("  [Constraints] EnzConstraintIO applied successfully (safe clone)")
                counts = {}
                if pose.constraint_set().has_constraints():
                    for cst in pose.constraint_set().get_all_constraints():
                        name = cst.__class__.__name__
                        counts[name] = counts.get(name, 0) + 1
                print(f"  [Constraints] Post-EnzConstraintIO constraint summary: {counts if counts else 'none'}")
            except Exception as e:
                print(f"  [Constraints] Warning: failed to summarize constraints: {e}")
        except RuntimeError as e:
            # Adding enzyme constraints can fail if pseudobonds already exist.
            # Do not hard-fail the pipeline; log and continue.
            print("  [Constraints] WARNING: EnzConstraintIO failed to add constraints.")
            print(f"  [Constraints] Error: {type(e).__name__}: {e}")
            print("  [Constraints] Continuing without re-adding enzyme constraints.")
            self._enzcst_failed = True
...
def run_secondary_mpnn(pose, pdbname, suffix, n_iter, i, ligand_seqpos, heavyatoms, keep_pos,
                       catres, cst_manager, sfx, mpnn_config, temperatures=None, outdir=None):
    ...
    seqs_dir = os.path.join(outdir, "seqs") if outdir else "seqs"
    ...
    for T in temperatures:
        ...
        result = run_mpnn_from_pose(pose, mpnn_config, seqs_dir, name, temperature=T,
        ...
        for seqid, output_file in enumerate(result["output_files"]):
            try:
                ...
                sfx(packed_pose)
                packed_pose.dump_pdb(os.path.join(seqs_dir, f"{pdbname}{suffix}_{n_iter}_{i}_T{T_str}_{seqid}.pdb"))
            except Exception as e:
                print(f"  [2nd Layer MPNN] WARNING: Failed to process {output_file}: {e}")


def _build_pdb_with_remark666(pose, remark666_lines, header_lines=None):
    """
    Build a PDB string with restored REMARK 666 lines and optional headers.
    """
    ...
    final_lines = headers + (remark666_lines or []) + protein_lines + hetatm_lines
...
def run_design_iteration(pose, fmd, cst_manager, scoring_module, pdbname, suffix, n_iter,
                         ...
                         original_remark666_lines=None, original_header_lines=None,
                         outdir=None, debug=False, input_pdb_path=None, ref_pdb_path=None,
                         cstfile=None, constraint_remark666_lines=None):
    ...
    for i, p in enumerate(poses):
        ...
        output_base = os.path.join(outdir, output_name) if outdir else output_name

        if cst_manager is not None:
            pre_counts = {}
            if debug:
                try:
                    for cst in p.constraint_set().get_all_constraints():
                        name = cst.__class__.__name__
                        pre_counts[name] = pre_counts.get(name, 0) + 1
                    if pre_counts:
                        print(f"  [CST] Pre-add constraint counts: {pre_counts}")
                except Exception as e:
                    print(f"  [CST] Warning: failed to enumerate pre-add constraints: {e}")

            enzyme_types = {"AtomPairConstraint", "AngleConstraint", "DihedralConstraint"}
            if pre_counts and (enzyme_types & set(pre_counts.keys())):
                if debug:
                    print("  [CST] Enzyme constraints already present; skipping re-add.")
            else:
                if pre_counts and debug:
                    print("  [CST] No enzyme constraints detected; attempting EnzConstraintIO add.")
                cst_manager.add_constraints(p)

            if debug:
                has_cst = p.constraint_set().has_constraints()
                print(f"  [CST] Constraints present after add: {has_cst}")
                try:
                    cst_counts = {}
                    for cst in p.constraint_set().get_all_constraints():
                        name = cst.__class__.__name__
                        cst_counts[name] = cst_counts.get(name, 0) + 1
                    if cst_counts:
                        print(f"  [CST] Constraint counts: {cst_counts}")
                except Exception as e:
                    print(f"  [CST] Warning: failed to enumerate constraints: {e}")

        # Use the configured scorefunction with constraint weights (not the default fa_scorefxn)
        # This ensures constraint scores (atom_pair, angle, dihedral) are properly calculated
        scores_df = scoring_module.score_design(p, sfx, list(catres.keys()))
        if debug:
            try:
                counts = {}
                if p.constraint_set().has_constraints():
                    for cst in p.constraint_set().get_all_constraints():
                        name = cst.__class__.__name__
                        counts[name] = counts.get(name, 0) + 1
                enzcst_failed = getattr(cst_manager, "_enzcst_failed", False) if cst_manager else False
                print(f"  [CST] Final constraint summary (post-scoring): {counts if counts else 'none'}")
                print(f"  [CST] EnzConstraintIO failed earlier: {enzcst_failed}")
            except Exception as e:
                print(f"  [CST] Warning: failed to summarize post-scoring constraints: {e}")

        enzyme_types = {"AtomPairConstraint", "AngleConstraint", "DihedralConstraint"}
        has_enzyme_cst = False
        try:
            if p.constraint_set().has_constraints():
                existing = {c.__class__.__name__ for c in p.constraint_set().get_all_constraints()}
                has_enzyme_cst = bool(existing & enzyme_types)
        except Exception:
            has_enzyme_cst = False

        if cstfile:
            manual_scores = rosetta_utils.evaluate_cstfile_constraints(
                p, cstfile, constraint_remark666_lines or [], verbose=debug
            )
            if manual_scores:
                totals = {resno: info.get("total", 0.0) for resno, info in manual_scores.items()}
                scores_df.at[0, "cstfile_total"] = sum(totals.values())
                scores_df.at[0, "cstfile_mean"] = float(np.mean(list(totals.values())))
                scores_df.at[0, "cstfile_max"] = float(np.max(list(totals.values())))
                worst_resno = max(totals, key=totals.get)
                scores_df.at[0, "cstfile_worst_resno"] = worst_resno
                scores_df.at[0, "cstfile_worst_value"] = totals[worst_resno]
                for resno, value in totals.items():
                    scores_df.at[0, f"cstfile_res_{resno}"] = value
                if debug:
                    print(f"  [CST] CSTfile max={scores_df.at[0, 'cstfile_max']:.2f} at res {worst_resno}")

                if not has_enzyme_cst:
                    scores_df.at[0, "catres_cst_total"] = scores_df.at[0, "cstfile_total"]
                    scores_df.at[0, "catres_cst_mean"] = scores_df.at[0, "cstfile_mean"]
                    scores_df.at[0, "catres_cst_max"] = scores_df.at[0, "cstfile_max"]
                    scores_df.at[0, "catres_cst_worst_resno"] = scores_df.at[0, "cstfile_worst_resno"]
                    scores_df.at[0, "catres_cst_worst_value"] = scores_df.at[0, "cstfile_worst_value"]
                    for resno, value in totals.items():
                        scores_df.at[0, f"catres_cst_{resno}"] = value
        if cst_manager is not None and has_enzyme_cst:
            cat_scores = rosetta_utils.evaluate_catalytic_constraint_scores(
                p, sfx, list(catres.keys()), verbose=debug
            )
            if cat_scores:
                per_res_totals = {resno: info.get("total", 0.0) for resno, info in cat_scores.items()}
                scores_df.at[0, "catres_cst_total"] = sum(per_res_totals.values())
                scores_df.at[0, "catres_cst_mean"] = float(np.mean(list(per_res_totals.values())))
                scores_df.at[0, "catres_cst_max"] = float(np.max(list(per_res_totals.values())))
                worst_resno = max(per_res_totals, key=per_res_totals.get)
                scores_df.at[0, "catres_cst_worst_resno"] = worst_resno
                scores_df.at[0, "catres_cst_worst_value"] = per_res_totals[worst_resno]
                scores_df.at[0, "catres_cst_n_above_1"] = sum(1 for v in per_res_totals.values() if v > 1.0)
                scores_df.at[0, "catres_cst_n_above_2"] = sum(1 for v in per_res_totals.values() if v > 2.0)
                for resno, value in per_res_totals.items():
                    scores_df.at[0, f"catres_cst_{resno}"] = value
                if debug:
                    print(f"  [CST] Catres CST max={scores_df.at[0, 'catres_cst_max']:.2f} at res {worst_resno}")
        ...
        if ref_pose is not None and ref_catres is not None:
            try:
                ...
                if debug and "lig_rmsd_to_refpdb" in ref_metrics:
                    print(f"  [Ref PDB] lig_rmsd_to_refpdb (high precision): {ref_metrics['lig_rmsd_to_refpdb']:.9f} Å")
            except Exception as e:
                print(f"  [WARNING] Failed to calculate ref_pdb metrics: {e}")

        if not cstfile and ref_pose is not None and ref_catres is not None:
            ref_rmsd = rosetta_utils.calculate_catalytic_residue_rmsd(
                p, ref_pose, ref_catres, atoms="functional", verbose=debug
            )
            if ref_rmsd and ref_rmsd.get("per_residue"):
                per_res = ref_rmsd["per_residue"]
                totals = {resno: info.get("rmsd", 0.0) for resno, info in per_res.items()}
                scores_df.at[0, "refcatres_total"] = sum(totals.values())
                scores_df.at[0, "refcatres_mean"] = float(np.mean(list(totals.values())))
                scores_df.at[0, "refcatres_max"] = float(np.max(list(totals.values())))
                worst_resno = max(totals, key=totals.get)
                scores_df.at[0, "refcatres_worst_resno"] = worst_resno
                scores_df.at[0, "refcatres_worst_value"] = totals[worst_resno]
                for resno, value in totals.items():
                    scores_df.at[0, f"refcatres_res_{resno}"] = value
                if debug:
                    print(f"  [Ref PDB] Refcatres max={scores_df.at[0, 'refcatres_max']:.3f} at res {worst_resno}")
        ...
        pdb_path = f"{output_base}.pdb"
        scores_df.at[0, "pdb_path"] = pdb_path
        scores_df.at[0, "input_pdb"] = input_pdb_path or ""
        scores_df.at[0, "ref_pdb"] = ref_pdb_path or ""
        if "description" in scores_df.columns:
            scores_df = scores_df.drop(columns=["description"])
        rosetta_utils.dump_scorefile(scores_df, scorefilename)
        if original_remark666_lines:
            validation = rosetta_utils.validate_remark666_consistency(
                p, remark666_lines=original_remark666_lines, verbose=True
            )
            if not validation['valid']:
                raise RuntimeError(
                    f"REMARK 666 validation failed for {output_name}.pdb: "
                    f"{validation.get('errors', [])}"
                )

        if original_remark666_lines:
            pdb_out = _build_pdb_with_remark666(p, original_remark666_lines, original_header_lines)
            with open(f"{output_base}.pdb", "w") as f:
                f.write(pdb_out)
            print(f"  [REMARK666] Wrote output with {len(original_remark666_lines)} REMARK 666 lines")
        else:
            p.dump_pdb(f"{output_base}.pdb")
        saved_count += 1
        ...
        if input_pose is not None and ref_pose is not None:
            ...
            print(f"  [SAVED] {output_base}.pdb (CA_rmsd={ca_rmsd:.2f}Å, seq_id={seq_id:.1%}, lig_rmsd_ref={lig_rmsd_ref:.3f}Å)")
        elif input_pose is not None:
            print(f"  [SAVED] {output_base}.pdb (CA_rmsd={ca_rmsd:.2f}Å, seq_id={seq_id:.1%})")
        else:
            print(f"  [SAVED] {output_base}.pdb")

        if run_mpnn and mpnn_config is not None:
            run_secondary_mpnn(p, pdbname, suffix, n_iter, i, ligand_seqpos, heavyatoms,
                               keep_pos, catres, cst_manager, sfx, mpnn_config, outdir=outdir)
...
def parse_arguments():
    ...
    parser.add_argument("--outdir", type=str, default=None, help="Output directory root (default: current working directory)")
    ...
    parser.add_argument("--catres_coord_cst", action="store_true", default=True, help="Enable catalytic residue coordinate constraints (default: enabled)")
    parser.add_argument("--no_catres_coord_cst", action="store_true", default=False, help="Disable catalytic residue coordinate constraints")
    parser.add_argument("--catres_coord_cst_sd", type=float, default=1.0, help="SD for catalytic residue coordinate constraints in Angstroms (default: 1.0)")
...
def main():
    ...
    if args.debug:
        debug_dir = os.path.join(outdir, f"debug_output_{pdbname}{suffix}")
        ...
        print(f"[DEBUG] Tracker output directory: {debug_dir}")
        print(f"[DEBUG] Structures directory: {debug_structures_dir}")

    # Setup directories
    scores_dir = os.path.join(outdir, "scores")
    os.makedirs(scores_dir, exist_ok=True)
    scorefilename = os.path.join(scores_dir, f"{pdbname}{suffix}.sc")
    if args.second_shell_mpnn_seqs:
        os.makedirs(os.path.join(outdir, "seqs"), exist_ok=True)
    ...
    original_pdb_lines = open(args.pdb, "r").read().splitlines()
    original_remark666_lines = [l for l in original_pdb_lines if l.startswith("REMARK 666")]
    original_header_lines = [
        l for l in original_pdb_lines
        if (l.startswith(("HEADER", "HETNAM", "LINK", "REMARK")) and not l.startswith("REMARK 666"))
    ]
    print(f"  Original REMARK 666 lines: {len(original_remark666_lines)}")
    ...
    constraint_remark666_lines = original_remark666_lines
    if args.cstfile:
        ...
        constraint_remark666_lines = hydrogen_utils.extract_remark666_lines(pdb_to_load)
        print(f"  [CST] Filtered REMARK 666 lines for constraints: {len(constraint_remark666_lines)}")
        try:
            blocks = []
            parse_failures = 0
            for line in constraint_remark666_lines:
                parts = line.split()
                try:
                    block = int(parts[-2])
                    blocks.append(block)
                    ...
                    parse_failures += 1
            if blocks:
                print(f"  [CST] Filtered REMARK 666 block numbers: {sorted(set(blocks))}")
            if parse_failures:
                print(f"  [CST] WARNING: Failed to parse {parse_failures} REMARK 666 lines for block numbers")
        except Exception as e:
            print(f"  [CST] WARNING: Failed block-number summary: {e}")
    ...
    # Map ORIGINAL REMARK 666 residues to current pose seqpos for fixing/mutation checks
    parsed_original_catres = hydrogen_utils.parse_catalytic_residues_from_remark666(
        original_remark666_lines, verbose=True
    )
    pose_res_map = {}
    for seqpos in range(1, pose.size() + 1):
        if pose.residue(seqpos).is_protein():
            key = (pose.pdb_info().chain(seqpos), pose.pdb_info().number(seqpos))
            pose_res_map[key] = seqpos

    original_catres_seqpos = []
    missing_catres = []
    for key, info in parsed_original_catres.items():
        if key in pose_res_map:
            original_catres_seqpos.append(pose_res_map[key])
        else:
            missing_catres.append(f"{key[0]}{key[1]}")

    if missing_catres:
        raise RuntimeError(
            f"[REMARK666] Failed to map catalytic residues to pose: {missing_catres}"
        )
    print(f"  [REMARK666] Mapped {len(original_catres_seqpos)} catalytic residues to pose seqpos")
    ...
    keep_pos += original_catres_seqpos
    print(f"  [REMARK666] Forcing {len(original_catres_seqpos)} catalytic residues to be fixed")
...
"""
    ...
    for n_iter in range(args.nstruct):
        existing = glob.glob(os.path.join(outdir, f"{pdbname}{suffix}_{n_iter}_*.pdb"))
        if existing:
        ...
        fmd.set_original_remark666_lines(original_remark666_lines)
        fmd.set_constraint_remark666_lines(constraint_remark666_lines)
        if args.no_catres_coord_cst:
            fmd.set_catres_coord_cst(False, sd=args.catres_coord_cst_sd)
            print("  [CatRes Coord CST] Disabled via CLI")
        else:
            fmd.set_catres_coord_cst(True, sd=args.catres_coord_cst_sd)
            print(f"  [CatRes Coord CST] Enabled via CLI (sd={args.catres_coord_cst_sd} A)")
        ...
        # Tracker outputs go to the parent debug dir, structures to debug_structures_dir
        # Note: debug_structures_dir is already set in FastMPNNdesign constructor (line 1286)
        if args.debug:
            tracker = pipeline_tracker.PipelineTracker(
                output_dir=debug_dir,  # Use parent debug dir for tracker outputs
                verbose=True
            )
            fmd.set_tracker(tracker)
            # Do NOT call fmd.set_debug_output_dir() here - it's already set correctly to debug_structures_dir
            print(f"  [DEBUG] Pipeline tracker enabled")
            print(f"  [DEBUG] Tracker output directory: {debug_dir}")
            print(f"  [DEBUG] Structures directory: {debug_structures_dir}")
        ...
        saved = run_design_iteration(
            ...
            original_remark666_lines=original_remark666_lines,
            original_header_lines=original_header_lines,
            outdir=outdir,
            debug=args.debug,
            input_pdb_path=args.pdb,
            ref_pdb_path=args.ref_pdb,
            cstfile=args.cstfile,
            constraint_remark666_lines=constraint_remark666_lines
        )
