...
## Bug 5: EnzConstraintIO Failure + Packer CountPair Assertion (ACTIVE)

**Symptoms:**
- EnzConstraintIO fails during MPNN pose setup:
  ```
  [Covalent] Warning: EnzConstraintIO failed: RuntimeError:
  File: core/conformation/Residue.cc:600
  ```
- Repack crashes immediately after MPNN:
  ```
  ERROR: Assertion `res2.is_bonded(res1)` failed.
  File: core/scoring/etable/count_pair/CountPairFactory.cc:220
  ```

**Observed Context (from logs):**
- Metal coordination variants are present (e.g., `HIS_D:MP-NE2-connect`, `LYS:N-conjugated:MP-NZ-connect`)
- EnzConstraintIO fails after pseudobonds are created
- Packer crash occurs on the first repack with only coordinate constraints present

**Likely Root Cause:**
- EnzConstraintIO attempts to alter residue connections in the presence of pseudobonds, which is prohibited by Rosetta.
- The failure can leave the pose in a partially modified state (inconsistent connection graph), which then triggers the packer assertion in `CountPairFactory.cc`.

**Implemented Mitigation (Safe-Clone Constraint Application):**
- EnzConstraintIO is applied to a **clone** of the pose.
- If it succeeds, the clone replaces the original.
- If it fails, the original pose is kept unchanged (avoids partial corruption).
- This prevents the packer from crashing due to partial EnzConstraintIO modifications.

**Debugging Signals to Watch (in `full_log.txt`):**
- `[Covalent] Warning: EnzConstraintIO failed: ...`
- `[Covalent] Keeping original pose unchanged after EnzConstraintIO failure`
- `[CST] Pre-EnzConstraintIO constraint summary: ...`
- `[CST] Post-EnzConstraintIO constraint summary: ...`
- `[FastMPNN] repack - pre-apply constraint summary: ...`
- `[FastMPNN] repack - post-apply constraint summary: ...`

**Suggested Next Tests / Hypotheses:**
1. **Metal pseudobond interference**: If EnzConstraintIO fails consistently, test whether metal coordination is creating incompatible residue connections.
2. **Ligand metal atoms**: If Zn atoms are part of the ligand (XDW), Rosetta may auto-create pseudobonds; ensure the ligand params support this or disable metal auto-detection.
3. **REMARK 666 mismatch**: Confirm post-MPNN REMARK 666 lines match pose residue types; mismatches can cause EnzConstraintIO to attempt illegal residue changes.
4. **Connection graph integrity**: If repack still fails after the safe-clone change, add a connection integrity check and dump per-residue bonds.

**Workarounds (if needed):**
- Skip EnzConstraintIO for MPNN poses and rely on coordinate constraints + ref_pdb RMSD metrics (temporary).
- Strip metal-binding variants if no explicit bonds exist (advanced; requires careful testing).
