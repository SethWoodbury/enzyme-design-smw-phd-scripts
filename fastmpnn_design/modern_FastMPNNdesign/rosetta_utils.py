...
def reestablish_covalent_connections(pose, ref_pose, cst_io=None, sfx=None,
    ...
    def _summarize_constraints(target_pose, label):
        try:
            counts = {}
            if target_pose.constraint_set().has_constraints():
                for cst in target_pose.constraint_set().get_all_constraints():
                    name = cst.__class__.__name__
                    counts[name] = counts.get(name, 0) + 1
            print(f"  [CST] {label} constraint summary: {counts if counts else 'none'}")
        except Exception as e:
            print(f"  [CST] Warning: failed to summarize constraints: {e}")

    def _apply_enz_constraints_if_needed(target_pose):
        if cst_io is None or sfx is None:
            return target_pose
        try:
            enzyme_types = {"AtomPairConstraint", "AngleConstraint", "DihedralConstraint"}
            existing = set()
            if target_pose.constraint_set().has_constraints():
                existing = {c.__class__.__name__ for c in target_pose.constraint_set().get_all_constraints()}
            if existing & enzyme_types:
                if verbose:
                    print("  [Covalent] Enzyme constraints already present; skipping EnzConstraintIO.")
                return target_pose
        except Exception as e:
            if verbose:
                print(f"  [Covalent] Warning: failed to inspect constraints: {e}")
        if verbose:
            print("  [Covalent] Applying enzyme constraints via EnzConstraintIO...")
            _summarize_constraints(target_pose, "Pre-EnzConstraintIO")
        safe_pose = target_pose.clone()
        try:
            cst_io.add_constraints_to_pose(safe_pose, sfx, True)
        except Exception as e:
            if verbose:
                print(f"  [Covalent] Warning: EnzConstraintIO failed: {type(e).__name__}: {e}")
                print("  [Covalent] Keeping original pose unchanged after EnzConstraintIO failure")
            return target_pose
        if verbose:
            _summarize_constraints(safe_pose, "Post-EnzConstraintIO")
            print("  [Covalent] EnzConstraintIO applied on safe clone; using updated pose")
        return safe_pose
    ...
    if not ref_connections:
        if verbose:
            ...
            print("  [Covalent] Still applying enzyme constraints from CST/REMARK 666")
        pose = _apply_enz_constraints_if_needed(pose)
        return pose
...
def dump_connection_integrity(pose, label="", verbose=True):
    """
    Print all inter-residue connections and flag any asymmetries.

    This helps diagnose CountPairFactory assertion failures caused by
    broken residue connection graphs.
    """
    if not verbose:
        return
    print(f"  [ConnDump] Connection integrity dump: {label}")
    total = 0
    asym = 0
    try:
        for resno in range(1, pose.size() + 1):
            res = pose.residue(resno)
            try:
                n_conn = res.n_possible_residue_connections()
            except Exception:
                # Fallback to n_connections if available
                n_conn = res.n_connections()
            for conn_id in range(1, n_conn + 1):
                try:
                    partner = res.residue_connection_partner(conn_id)
                except Exception:
                    continue
                if partner == 0:
                    continue
                ...
                try:
                    ...
                    try:
                        partner_n = partner_res.n_possible_residue_connections()
                    except Exception:
                        partner_n = partner_res.n_connections()
                    for p_conn in range(1, partner_n + 1):
                        try:
                            if partner_res.residue_connection_partner(p_conn) == resno:
                                back_found = True
                                break
                        except Exception:
                            continue
                    ...
                    pass
                tag = "ASYM" if not back_found else "OK"
                if not back_found:
                    asym += 1
                print(
                    f"  [ConnDump] {tag} {res.name3()}{resno}:{atom_name} -> "
                    f"{pose.residue(partner).name3()}{partner}"
                )
        print(f"  [ConnDump] Summary: connections={total}, asymmetries={asym}")
    except Exception as e:
        print(f"  [ConnDump] WARNING: failed to dump connections: {e}")


def pose_has_mp_coordination(pose):
    """
    Detect if any residue type name includes metal coordination patches.
    """
    details = []
    for resno in range(1, pose.size() + 1):
        res_type = pose.residue(resno).type().name()
        if "MP-" in res_type:
            details.append((resno, pose.residue(resno).name3(), res_type))
    return details


def _angle_degrees(p1, p2, p3):
    v1 = p1 - p2
    v2 = p3 - p2
...
def _dihedral_degrees(p1, p2, p3, p4):
    b0 = p2 - p1
    b1 = p3 - p2
    b2 = p4 - p3
...
def parse_cst_file(cstfile, verbose=False):
    """
    Parse a Rosetta enzyme CST file into structured blocks.
    """
    ...
    block_lines = []
    for line in lines:
        if "VARIABLE_CST::BEGIN" in line:
            in_block = True
            block_lines = []
            continue
        if "VARIABLE_CST::END" in line:
            if block_lines:
    ...
    for idx, block in enumerate(blocks, start=1):
        ...
        for line in block:
            if line.startswith("TEMPLATE::") and "ATOM_MAP:" in line:
                parts = line.replace(":", " ").split()
                # Expected: TEMPLATE:: ATOM_MAP 1 atom_name ZN1 O3 P1
                try:
                    map_idx = int(parts[parts.index("ATOM_MAP") + 1])
                except Exception:
                    continue
                if "atom_name" in parts:
                    i = parts.index("atom_name") + 1
                    atoms = parts[i:]
                    if map_idx == 1:
                        entry["map1_atoms"] = atoms
                    elif map_idx == 2:
                        entry["map2_atoms"] = atoms
                if "atom_type" in parts:
                    i = parts.index("atom_type") + 1
                    types = parts[i:]
                    if map_idx == 1:
                        entry["map1_types"] = types
                    elif map_idx == 2:
                        entry["map2_types"] = types
                if "residue3" in parts:
                    i = parts.index("residue3") + 1
                    res3 = parts[i] if i < len(parts) else None
                    if map_idx == 1:
                        entry["map1_res3"] = res3
                    elif map_idx == 2:
                        entry["map2_res3"] = res3
            elif line.startswith("CONSTRAINT::"):
                key_val = line.split("::", 1)[1].strip()
                if ":" not in key_val:
                    continue
                key, rest = key_val.split(":", 1)
                parts = rest.split()
                if len(parts) < 2:
                    continue
                try:
    ...
    if verbose:
        print(f"  [CST] Parsed {len(parsed)} CST blocks from {cstfile}")
    return parsed


def evaluate_cstfile_constraints(pose, cstfile, remark666_lines, verbose=True):
    """
    Evaluate CST file constraints manually as a fallback when EnzConstraintIO fails.
    Returns per-residue scores based on CST block definitions.
    """
    if not cstfile or not remark666_lines:
        return {}

    blocks = parse_cst_file(cstfile, verbose=verbose)
    if not blocks:
        return {}

    pdb_info = pose.pdb_info()
    chain_resno_to_seqpos = {}
    for resno in range(1, pose.size() + 1):
        chain_resno_to_seqpos[(pdb_info.chain(resno), pdb_info.number(resno))] = resno
    ...
    for line in remark666_lines:
        ...
        try:
            ...
            resno = int(parts[motif_idx + 3])
            block = int(parts[-2])
        except Exception:
            parse_failures += 1
            continue
        seqpos = chain_resno_to_seqpos.get((chain, resno))
        if seqpos is None:
            continue
        block_to_seqpos[block] = seqpos
        block_to_resname[block] = resname

    if verbose and parse_failures:
        print(f"  [CST] WARNING: Failed to parse {parse_failures} REMARK 666 lines for block mapping")

    ligand_resnos = get_ligand_residues(pose)
    ligand_seqpos = ligand_resnos[0] if ligand_resnos else None
    if ligand_seqpos is None:
        if verbose:
            print("  [CST] WARNING: No ligand found for CST evaluation")
        return {}

    def resolve_atoms(res, atoms, types):
        if atoms:
            return [a for a in atoms if res.has(a)]
        if types:
            res_name3 = res.name3()
            if res_name3 in FUNCTIONAL_ATOMS:
                return [a for a in FUNCTIONAL_ATOMS[res_name3] if res.has(a)]
            # Fallback: all sidechain heavy atoms
            resolved = []
            for i in range(1, res.natoms() + 1):
                if not res.atom_is_backbone(i) and not res.atom_is_hydrogen(i):
                    resolved.append(res.atom_name(i).strip())
            return resolved
    ...
    for entry in blocks:
        ...
        seqpos = block_to_seqpos.get(block_id)
        if seqpos is None:
            if verbose:
                print(f"  [CST] WARNING: No REMARK 666 mapping for CST block {block_id}")
            continue

        lig_res = pose.residue(ligand_seqpos)
        prot_res = pose.residue(seqpos)

        map1_atoms = resolve_atoms(lig_res, entry["map1_atoms"], entry["map1_types"])
        map2_atoms = resolve_atoms(prot_res, entry["map2_atoms"], entry["map2_types"])

        if verbose:
            print(f"  [CST] Block {block_id} -> {prot_res.name3()}{seqpos} (ligand {lig_res.name3()}{ligand_seqpos})")
            print(f"  [CST]   Map1 atoms: {map1_atoms}")
            print(f"  [CST]   Map2 atoms: {map2_atoms}")

        if not map1_atoms or not map2_atoms:
            if verbose:
                print(f"  [CST] WARNING: Missing atom mappings for block {block_id}")
            continue
        ...
        # distanceAB
        if entry["distanceAB"] is not None:
            target, sd = entry["distanceAB"]
            best = None
            best_pair = None
            for a1 in map1_atoms:
                for b1 in map2_atoms:
                    d = np.linalg.norm(lig_res.xyz(a1) - prot_res.xyz(b1))
                    dev = abs(d - target)
                    score = (dev / sd) ** 2 if sd > 0 else dev
                    if best is None or score < best:
                        best = score
                        best_pair = (a1, b1, d, dev)
            dist_score = best if best is not None else 0.0
            if verbose and best_pair:
                a1, b1, d, dev = best_pair
                print(f"  [CST]   distanceAB {a1}-{b1}: d={d:.3f} target={target:.3f} dev={dev:.3f} score={dist_score:.3f}")

        # angle_A
        if entry["angle_A"] is not None:
            target, sd = entry["angle_A"]
            best = None
            best_trip = None
            for a1 in map1_atoms:
                for a2 in map1_atoms:
                    if a2 == a1:
                        continue
                    for b1 in map2_atoms:
                        ang = _angle_degrees(lig_res.xyz(a2), lig_res.xyz(a1), prot_res.xyz(b1))
                        dev = abs(ang - target)
                        score = (dev / sd) ** 2 if sd > 0 else dev
                        if best is None or score < best:
                            best = score
                            best_trip = (a2, a1, b1, ang, dev)
            ang_score += best if best is not None else 0.0
            if verbose and best_trip:
                a2, a1, b1, ang, dev = best_trip
                print(f"  [CST]   angle_A {a2}-{a1}-{b1}: ang={ang:.2f} target={target:.2f} dev={dev:.2f} score={best:.3f}")

        # angle_B
        if entry["angle_B"] is not None:
            target, sd = entry["angle_B"]
            best = None
            best_trip = None
            for a1 in map1_atoms:
                for b1 in map2_atoms:
                    for b2 in map2_atoms:
                        if b2 == b1:
                            continue
                        ang = _angle_degrees(lig_res.xyz(a1), prot_res.xyz(b1), prot_res.xyz(b2))
                        dev = abs(ang - target)
                        score = (dev / sd) ** 2 if sd > 0 else dev
                        if best is None or score < best:
                            best = score
                            best_trip = (a1, b1, b2, ang, dev)
            ang_score += best if best is not None else 0.0
            if verbose and best_trip:
                a1, b1, b2, ang, dev = best_trip
                print(f"  [CST]   angle_B {a1}-{b1}-{b2}: ang={ang:.2f} target={target:.2f} dev={dev:.2f} score={best:.3f}")

        # torsion_A
        if entry["torsion_A"] is not None:
            target, sd = entry["torsion_A"]
            best = None
            best_quad = None
            for a1 in map1_atoms:
                for a2 in map1_atoms:
                    for a3 in map1_atoms:
                        if len({a1, a2, a3}) < 3:
                            continue
                        for b1 in map2_atoms:
                            dih = _dihedral_degrees(lig_res.xyz(a3), lig_res.xyz(a2), lig_res.xyz(a1), prot_res.xyz(b1))
                            dev = _angular_diff_deg(dih, target)
                            score = (dev / sd) ** 2 if sd > 0 else dev
                            if best is None or score < best:
                                best = score
                                best_quad = (a3, a2, a1, b1, dih, dev)
            dih_score += best if best is not None else 0.0
            if verbose and best_quad:
                a3, a2, a1, b1, dih, dev = best_quad
                print(f"  [CST]   torsion_A {a3}-{a2}-{a1}-{b1}: dih={dih:.2f} target={target:.2f} dev={dev:.2f} score={best:.3f}")

        # torsion_AB
        if entry["torsion_AB"] is not None:
            target, sd = entry["torsion_AB"]
            best = None
            best_quad = None
            for a1 in map1_atoms:
                for a2 in map1_atoms:
                    ...
                    for b1 in map2_atoms:
                        for b2 in map2_atoms:
                            if b2 == b1:
                                continue
                            dih = _dihedral_degrees(lig_res.xyz(a2), lig_res.xyz(a1), prot_res.xyz(b1), prot_res.xyz(b2))
                            dev = _angular_diff_deg(dih, target)
                            score = (dev / sd) ** 2 if sd > 0 else dev
                            if best is None or score < best:
                                best = score
                                best_quad = (a2, a1, b1, b2, dih, dev)
            dih_score += best if best is not None else 0.0
            if verbose and best_quad:
                a2, a1, b1, b2, dih, dev = best_quad
                print(f"  [CST]   torsion_AB {a2}-{a1}-{b1}-{b2}: dih={dih:.2f} target={target:.2f} dev={dev:.2f} score={best:.3f}")

        # torsion_B
        if entry["torsion_B"] is not None:
            target, sd = entry["torsion_B"]
            best = None
            best_quad = None
            for a1 in map1_atoms:
                for b1 in map2_atoms:
                    for b2 in map2_atoms:
                        for b3 in map2_atoms:
                            if len({b1, b2, b3}) < 3:
                                continue
                            dih = _dihedral_degrees(lig_res.xyz(a1), prot_res.xyz(b1), prot_res.xyz(b2), prot_res.xyz(b3))
                            dev = _angular_diff_deg(dih, target)
                            score = (dev / sd) ** 2 if sd > 0 else dev
                            if best is None or score < best:
                                best = score
                                best_quad = (a1, b1, b2, b3, dih, dev)
            dih_score += best if best is not None else 0.0
            if verbose and best_quad:
                a1, b1, b2, b3, dih, dev = best_quad
                print(f"  [CST]   torsion_B {a1}-{b1}-{b2}-{b3}: dih={dih:.2f} target={target:.2f} dev={dev:.2f} score={best:.3f}")

        total = dist_score + ang_score + dih_score
        per_res = per_res_scores.setdefault(seqpos, {"total": 0.0, "distance": 0.0, "angle": 0.0, "dihedral": 0.0})
        per_res["total"] += total
        per_res["distance"] += dist_score
        per_res["angle"] += ang_score
        per_res["dihedral"] += dih_score
...
def setup_pose_from_mpnn_output(mpnn_pdb_str, ref_pose, cst_io=None, sfx=None,
    ...
    if verbose:
        ...
        if not remark666_lines:
            print("  [REMARK666] WARNING: No REMARK 666 lines found in MPNN output PDB")
        else:
            remark_blocks = []
            parse_failures = 0
            for line in remark666_lines:
                ...
                try:
                    ...
                    chain = parts[motif_idx + 1]
                    resname = parts[motif_idx + 2]
                    resno = parts[motif_idx + 3]
                    block = parts[-2]
                    var = parts[-1]
                    remark_blocks.append(int(block))
                    print(f"  [REMARK666] MPNN line: chain={chain} res={resname}{resno} block={block} var={var}")
                except Exception:
                    parse_failures += 1
            if remark_blocks:
                print(f"  [REMARK666] MPNN block numbers: {sorted(set(remark_blocks))}")
            if parse_failures:
                print(f"  [REMARK666] WARNING: Failed to parse {parse_failures} REMARK 666 lines")
    ...
    if verbose and remark666_lines:
        try:
            pdb_info = pose.pdb_info()
            key_to_seqpos = {}
            for resno in range(1, pose.size() + 1):
                key_to_seqpos[(pdb_info.chain(resno), pdb_info.number(resno))] = resno
            print("  [REMARK666] Post-load residue type check:")
            for line in remark666_lines:
                parts = line.split()
                try:
                    motif_idx = parts.index("MOTIF")
                    ...
                    resname = parts[motif_idx + 2]
                    resno = int(parts[motif_idx + 3])
                    seqpos = key_to_seqpos.get((chain, resno))
                    if seqpos is None:
                        print(f"  [REMARK666] WARNING: {chain}{resno} ({resname}) not found in pose")
                        continue
                    pose_res = pose.residue(seqpos)
                    pose_name3 = pose_res.name3()
                    pose_type = pose_res.type().name()
                    print(f"  [REMARK666] {chain}{resno} expected={resname} pose={pose_name3} type={pose_type}")
                except Exception as e:
                    print(f"  [REMARK666] WARNING: Failed to parse line for pose check: {e}")
        except Exception as e:
            print(f"  [REMARK666] WARNING: Failed post-load REMARK 666 validation: {e}")
    ...
    if ref_pdb_pose is not None:
        pose = reestablish_covalent_connections(
            pose, ref_pdb_pose,
                    ...
                    try:
                        ...
                        if pose.constraint_set().has_constraints():
                            for cst in pose.constraint_set().get_all_constraints():
                                name = cst.__class__.__name__
                                counts[name] = counts.get(name, 0) + 1
                        print(f"  [CST] Pre-EnzConstraintIO constraint summary: {counts if counts else 'none'}")
                    except Exception as e:
                        print(f"  [CST] Warning: failed to summarize constraints: {e}")
                    ...
                    try:
                        ...
                        if pose.constraint_set().has_constraints():
                            for cst in pose.constraint_set().get_all_constraints():
                                name = cst.__class__.__name__
                                counts[name] = counts.get(name, 0) + 1
                        print(f"  [CST] Post-EnzConstraintIO constraint summary: {counts if counts else 'none'}")
                    except Exception as e:
                        print(f"  [CST] Warning: failed to summarize constraints: {e}")
            except Exception as e:
                if verbose:
    ...
    if verbose:
        ...
        if debug_output_dir:
            dump_connection_integrity(pose, label=f"{debug_prefix}final_pose", verbose=True)
