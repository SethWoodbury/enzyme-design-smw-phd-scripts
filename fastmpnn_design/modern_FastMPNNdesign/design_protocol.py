...
class FlipHistory:
    """
    Track chi angles and CST scores per residue over iterations for flip gating.

    This class maintains a history of chi angles and constraint scores for each
    catalytic residue, enabling convergence detection and persistent poor CST
    identification for evidence-based sidechain flips.
    """

    def __init__(self, convergence_window=3, chi_delta_thresh=5.0,
                 cst_persistence_window=3, cst_threshold=2.0):
        """
        Initialize flip history tracker.

        Arguments:
            convergence_window: Number of consecutive steps for chi convergence (default 3)
            chi_delta_thresh: Maximum chi angle change (degrees) to be considered converged (default 5.0)
            cst_persistence_window: Number of consecutive steps for CST persistence (default 3)
            cst_threshold: CST score threshold for "poor" constraint satisfaction (default 2.0)
        """
        self.convergence_window = convergence_window
        self.chi_delta_thresh = chi_delta_thresh
        self.cst_persistence_window = cst_persistence_window
        self.cst_threshold = cst_threshold

        # Per-residue tracking: {seqpos: {'chi': [list of chi angles], 'cst': [list of cst scores]}}
        self.residue_history = {}

    def record(self, pose_id, seqpos, chi_angles, cst_score):
        """
        Record chi angles and CST score for a residue at current iteration.

        Arguments:
            seqpos: Residue sequence position
            chi_angles: List of chi angles for the residue
            cst_score: Total constraint score for this residue
        """
        key = (pose_id, seqpos)
        if key not in self.residue_history:
            self.residue_history[key] = {'chi': [], 'cst': []}

        self.residue_history[key]['chi'].append(list(chi_angles))
        self.residue_history[key]['cst'].append(cst_score)

    def is_chi_converged(self, pose_id, seqpos):
        """
        Check if chi angles have converged for a residue.

        Returns True if the chi angle changes have been below the threshold
        for the required number of consecutive steps.

        Arguments:
            seqpos: Residue sequence position

        Returns:
            bool: True if chi angles are converged
        """
        key = (pose_id, seqpos)
        if key not in self.residue_history:
            return False

        chi_history = self.residue_history[key]['chi']
        if len(chi_history) < self.convergence_window + 1:
            return False

        # Check the last convergence_window steps
        for i in range(1, self.convergence_window + 1):
            chi_current = chi_history[-i]
            chi_prev = chi_history[-i - 1]

            # Compare each chi angle
            for j in range(min(len(chi_current), len(chi_prev))):
                delta = abs(chi_current[j] - chi_prev[j])
                # Handle wrap-around at +/- 180
                if delta > 180:
                    delta = 360 - delta
                if delta > self.chi_delta_thresh:
                    return False

        return True

    def is_cst_persistent(self, pose_id, seqpos):
        """
        Check if constraint scores have been persistently poor for a residue.

        Returns True if the CST score has been above the threshold for the
        ...
        Returns:
            bool: True if CST is persistently poor
        """
        key = (pose_id, seqpos)
        if key not in self.residue_history:
            return False

        cst_history = self.residue_history[key]['cst']
        if len(cst_history) < self.cst_persistence_window:
            return False

        # Check the last cst_persistence_window steps
        for i in range(1, self.cst_persistence_window + 1):
            if cst_history[-i] < self.cst_threshold:
                return False

        return True

    def get_summary(self, pose_id, seqpos):
        """
        Get a summary of the history for a residue.

        Arguments:
            seqpos: Residue sequence position

        Returns:
            dict: Summary including convergence status, persistence status, and recent values
        """
        key = (pose_id, seqpos)
        if key not in self.residue_history:
            return {'converged': False, 'persistent_poor_cst': False, 'n_records': 0}

        history = self.residue_history[key]
        return {
            'converged': self.is_chi_converged(pose_id, seqpos),
            'persistent_poor_cst': self.is_cst_persistent(pose_id, seqpos),
            'n_records': len(history['chi']),
            'recent_cst': history['cst'][-3:] if history['cst'] else [],
            'recent_chi': history['chi'][-1] if history['chi'] else []
        }
...
class FastMPNNdesign():
    def __init__(self, model_type=None, N_seq=5, params=None, name=None,
        ...
        self.__original_remark666_lines = None
        self.__constraint_remark666_lines = None
        ...
        self.__flip_cst_threshold = 2.0  # Constraint score threshold to trigger flip attempt (raised from 1.0)
        self.__flip_max_min_iter = 10  # Max iterations for brief local minimization after flip

        # Flip gating parameters - prevent early, uninformed flips
        # Flips require: stage gate + chi convergence + persistent CST violation
        self.__flip_stage_gate = 5  # No flips before protocol step N
        self.__flip_chi_delta_thresh = 5.0  # Max chi change (degrees) for convergence
        self.__flip_convergence_window = 3  # Steps required for chi convergence
        self.__flip_cst_persistence_window = 3  # Steps required for persistent poor CST
        self.__flip_torsion_ab_threshold = 120.0  # Min torsion_AB deviation (degrees) to trigger flip

        # Flip tracking for aggregate statistics
        self.__flip_tracker = FlipTracker()

        # Flip history for gating (tracks chi angles and CST per residue)
        self.__flip_history = FlipHistory(
            convergence_window=self.__flip_convergence_window,
            chi_delta_thresh=self.__flip_chi_delta_thresh,
            cst_persistence_window=self.__flip_cst_persistence_window,
            cst_threshold=self.__flip_cst_threshold
        )

        # Pipeline tracking for debugging - tracks catres RMSD and metrics at each stage
        self.__tracker = None  # Initialized via set_tracker() if debug enabled

        # HIS tautomer map from input PDB - preserves correct protonation states
        self.__his_tautomer_map = None  # Built from input PDB if provided
        self.__input_pdb_path = None  # Store input PDB path for tautomer detection

        # Catalytic residue coordinate constraints - prevent drift during minimization
        self.__catres_coord_cst_sd = 1.0  # Standard deviation for coordinate constraints (Angstroms)
        self.__enable_catres_coord_cst = True  # Add coordinate constraints to catalytic residues
    ...
    def set_allow_his_flips(self, allow: bool):
        """
        Deprecated no-op. Chemical sensitivity gating has been removed.
        """
        print("[FastMPNN] set_allow_his_flips() is deprecated (no effect).")

    def set_tracker(self, tracker: PipelineTracker):
        """
        Set the pipeline tracker for debugging and metric collection.
    ...
    def set_input_pdb_path(self, pdb_path: str):
        """
        Set the input PDB path for HIS tautomer detection.

        This enables detection of the correct HIS tautomers (HIS_D vs HIS)
        from the original PDB file, rather than relying on PyRosetta's
        auto-detection which may differ.

        Arguments:
            pdb_path: Path to the original input PDB file
        """
        self.__input_pdb_path = pdb_path
        if os.path.exists(pdb_path):
            self.__his_tautomer_map = hydrogen_utils.build_his_tautomer_map_from_pdb(pdb_path, verbose=True)

    def set_original_remark666_lines(self, remark666_lines):
        """
        Store original REMARK 666 lines from the input PDB for header restoration
        and validation, even if a CST subset is used internally.
        """
        self.__original_remark666_lines = list(remark666_lines) if remark666_lines else None

    def set_constraint_remark666_lines(self, remark666_lines):
        """
        Store filtered REMARK 666 lines used internally for constraint application.
        """
        self.__constraint_remark666_lines = list(remark666_lines) if remark666_lines else None

    def set_catres_coord_cst(self, enable: bool, sd: float = 0.5):
        """
        Configure coordinate constraints for catalytic residues.

        These constraints prevent catalytic residue drift during minimization.
        The standard CST file typically only constrains ligand-protein
        interactions, not the absolute positions of catalytic residues.

        Arguments:
            enable: Whether to add coordinate constraints
            sd: Standard deviation in Angstroms (smaller = tighter, default 0.5)
        """
        self.__enable_catres_coord_cst = enable
        self.__catres_coord_cst_sd = sd

    def set_debug_output_dir(self, output_dir: str):
        """
        Set the debug output directory for intermediate PDBs and reports.

        Arguments:
            output_dir: Path to directory for debug output
        """
        self.__debug_output_dir = output_dir
        self.__debug = True  # Also enable debug mode
    ...
    def set_catres_flip_config(self, enable_auto=True, cst_threshold=2.0, max_min_iter=10,
                                 stage_gate=5, chi_delta_thresh=5.0, convergence_window=3,
                                 cst_persistence_window=3, torsion_ab_threshold=120.0):
        """
        ...
        Flip gating (to prevent early, uninformed flips):
        - Stage gate: No flips before protocol step N
        - Chi convergence: Chi angles must be stable for convergence_window steps
        - CST persistence: CST must be above threshold for cst_persistence_window steps
        ...
        Arguments:
            ...
            stage_gate: No flips before this protocol step (default 5)
            chi_delta_thresh: Max chi angle change (degrees) for convergence (default 5.0)
            convergence_window: Steps required for chi convergence (default 3)
            cst_persistence_window: Steps required for persistent poor CST (default 3)
            torsion_ab_threshold: Min torsion_AB deviation (degrees) to trigger flip (default 120)
        """
        self.__enable_auto_flip = enable_auto
        self.__flip_cst_threshold = cst_threshold
        self.__flip_max_min_iter = max_min_iter
        self.__flip_stage_gate = stage_gate
        self.__flip_chi_delta_thresh = chi_delta_thresh
        self.__flip_convergence_window = convergence_window
        self.__flip_cst_persistence_window = cst_persistence_window
        self.__flip_torsion_ab_threshold = torsion_ab_threshold

        # Update FlipHistory with new parameters
        self.__flip_history = FlipHistory(
            convergence_window=convergence_window,
            chi_delta_thresh=chi_delta_thresh,
            cst_persistence_window=cst_persistence_window,
            cst_threshold=cst_threshold
        )

        status = "enabled" if enable_auto else "disabled"
        print(f"[FastMPNN] Catres sidechain flip: {status}, threshold={cst_threshold:.2f} REU, stage_gate={stage_gate}")
    ...
    def _fix_mpnn_packed_pdb(self, mpnn_pdb_str: str, ref_pdb_str: str) -> str:
        ...
        """
        # Extract header lines from reference (HEADER, HETNAM, LINK, non-666 REMARK)
        header_lines = []
        seen_headers = set()
        if self.__constraint_remark666_lines is not None:
            remark666_lines = list(self.__constraint_remark666_lines)
            print(f"[FastMPNN] Using {len(remark666_lines)} filtered REMARK 666 lines for constraints")
        else:
            remark666_lines = list(self.__original_remark666_lines) if self.__original_remark666_lines else []
            if self.__original_remark666_lines:
                print(f"[FastMPNN] Restoring {len(remark666_lines)} original REMARK 666 lines in MPNN output")

        for line in ref_pdb_str.split('\n'):
            if line.startswith("REMARK 666"):
                if not remark666_lines and line not in seen_headers:
        ...
        final_lines = header_lines + remark666_lines + protein_lines + hetatm_lines
    ...
    def do_mpnn(self, pose, mpnn_input, temperature, num_sequences):
        ...
        if self.__MPNN_pack_sc is True:
            ...
            for n in sorted(mpnn_out["packed"].keys()):
                ...
                # Debug: Save raw MPNN packed structure (before any modifications)
                if self.__debug and self.__debug_output_dir:
                    raw_mpnn_path = os.path.join(self.__debug_output_dir, f"mpnn_raw_packed_{len(poses)}.pdb")
                    with open(raw_mpnn_path, 'w') as f:
                        f.write(fixed_pdb_str)
                    print(f"  [DEBUG] Saved raw MPNN structure: {raw_mpnn_path}")

                # Use the new setup function that:
                # 1. Loads MPNN structure (preserving its geometry)
                # 2. Applies HIS tautomers BEFORE constraints (critical for metal-coordinating HIS)
                # 3. Re-establishes covalent connections ONLY if --ref_pdb was provided
                #    (filtered to catres-ligand bonds from catres_cst_subset)
                # 4. Adds tight coordinate constraints to ALL ligand atoms (including H) to keep ligand frozen
                # 5. Handles close contacts between ligand H and acid/base residues
                try:
                    # Prepare debug parameters
                    debug_dir = self.__debug_output_dir if self.__debug else None
                    debug_prefix = f"pose{len(poses)}_" if debug_dir else ""
                    ...
                    new_pose = rosetta_utils.setup_pose_from_mpnn_output(
                        ...
                        his_tautomer_map=self.__his_tautomer_map,  # Apply HIS tautomers before constraints
                        debug_output_dir=debug_dir,  # Pass debug directory for intermediate structures
                        debug_prefix=debug_prefix,   # Prefix for debug filenames
                        verbose=True
                    )

                    # Debug: Save structure after setup (with H, tautomers, REMARK 666) - REDUNDANT but kept for backwards compat
                    if self.__debug and self.__debug_output_dir:
                        setup_path = os.path.join(self.__debug_output_dir, f"mpnn_after_setup_{len(poses)}.pdb")
                        pdb_str_out = pyrosetta.distributed.io.to_pdbstring(new_pose)
                        with open(setup_path, 'w') as f:
                            f.write(pdb_str_out)
                        print(f"  [DEBUG] Saved setup structure: {setup_path}")
            ...
            poses = [self._thread_seq_to_pose(pose, seq) for seq in set(sequences)]
    ...
    def apply(self, pose):
        ...
        # Begin tracker stage for initialization if tracker is enabled
        if self.__tracker:
            self.__tracker.begin_stage("initialization", "Setting up FastMPNN design protocol")
        ...
        # Track initial catres RMSD if tracker and reference are available
        if self.__tracker and self.__catres_for_covalent and self.__ref_pdb_pose:
            rmsd_result = rosetta_utils.calculate_catalytic_residue_rmsd(
                pose, self.__ref_pdb_pose, self.__catres_for_covalent,
                atoms="functional", verbose=True
            )
            self.__tracker.log_catres_rmsd(rmsd_result, label="initial")
            self.__tracker.set_global_metric("n_catalytic_residues", len(self.__catres_for_covalent))
            self.__tracker.end_stage()
        ...
        for i, cmd in enumerate(self.script()):
            ...
            if command == "min":
                ...
                # NOTE: HIS tautomers are now applied INSIDE setup_pose_from_mpnn_output()
                # BEFORE constraints/pseudobonds are created. The duplicate call here was
                # causing crashes for metal-coordinating HIS residues.
                # See: rosetta_utils.py setup_pose_from_mpnn_output() and common_bugs_and_cautions.md

                # Add coordinate constraints for catalytic residues to prevent drift during minimization
                if self.__enable_catres_coord_cst and self.__catres_for_covalent and self.__ref_pdb_pose:
                    print(f"[FastMPNN] Adding catalytic residue coordinate constraints (sd={self.__catres_coord_cst_sd} A)")
                    for pi in poses:
                        rosetta_utils.add_catalytic_residue_coordinate_constraints(
                            poses[pi]["pose"],
                            self.__ref_pdb_pose,
                            self.__catres_for_covalent,
                            constraint_atoms="functional",
                            sd=self.__catres_coord_cst_sd,
                            verbose=True
                        )
                    # Ensure coordinate_constraint weight is set on scorefunction
                    coord_cst_st = pyrosetta.rosetta.core.scoring.score_type_from_name("coordinate_constraint")
                    if self.__scorefxn.get_weight(coord_cst_st) == 0.0:
                        self.__scorefxn.set_weight(coord_cst_st, 1.0)
                        print("[FastMPNN] Set coordinate_constraint weight to 1.0")

                # Track catres RMSD after MPNN if tracker enabled
                if self.__tracker and self.__catres_for_covalent and self.__ref_pdb_pose:
                    for pi in poses:
                        rmsd_result = rosetta_utils.calculate_catalytic_residue_rmsd(
                            poses[pi]["pose"], self.__ref_pdb_pose, self.__catres_for_covalent,
                            atoms="functional", verbose=True
                        )
                        self.__tracker.log_catres_rmsd(rmsd_result, label=f"post_mpnn_step{i}_pose{pi}")
            ...
            if command in ["repack", "min"]:
                ...
                for pi in poses:
                    ...
                    if self.__debug:
                        try:
                            counts = {}
                            if _p.constraint_set().has_constraints():
                                for cst in _p.constraint_set().get_all_constraints():
                                    name = cst.__class__.__name__
                                    counts[name] = counts.get(name, 0) + 1
                            print(f"[FastMPNN] {command} - pre-apply constraint summary: {counts if counts else 'none'}")
                        except Exception as e:
                            print(f"[FastMPNN] {command} - warning: failed to summarize pre-constraints: {e}")
                        rosetta_utils.dump_connection_integrity(_p, label=f"{command} pre-apply pose{pi}", verbose=True)
                        ...
                        try:
                            ...
                            if _p.constraint_set().has_constraints():
                                for cst in _p.constraint_set().get_all_constraints():
                                    name = cst.__class__.__name__
                                    counts[name] = counts.get(name, 0) + 1
                            print(f"[FastMPNN] {command} - post-apply constraint summary: {counts if counts else 'none'}")
                        except Exception as e:
                            print(f"[FastMPNN] {command} - warning: failed to summarize post-constraints: {e}")
                ...
                # Track catres RMSD after min/repack if tracker enabled
                if self.__tracker and self.__catres_for_covalent and self.__ref_pdb_pose:
                    for pi in poses:
                        rmsd_result = rosetta_utils.calculate_catalytic_residue_rmsd(
                            poses[pi]["pose"], self.__ref_pdb_pose, self.__catres_for_covalent,
                        ...
                        self.__tracker.log_catres_rmsd(rmsd_result, label=f"post_{command}_step{i}_pose{pi}")
                        # Warn if RMSD is high
                        if rmsd_result['overall_rmsd'] > 1.5:
                            print(f"[FastMPNN] WARNING: Catres RMSD {rmsd_result['overall_rmsd']:.2f} A > 1.5 A threshold!")

                # Automatic sidechain flip attempts for catalytic residues after min/repack
                # Flips are gated by: stage gate + chi convergence + persistent CST violation
                if self.__enable_auto_flip and self.__catres_for_covalent:
                    for pi in poses:
                        pose = poses[pi]["pose"]

                        # Update flip history with current chi angles and CST scores
                        cat_scores = rosetta_utils.evaluate_catalytic_constraint_scores(
                            pose, self.__scorefxn, self.__catres_for_covalent, verbose=False
                        )

                        for seqpos in self.__catres_for_covalent:
                            res = pose.residue(seqpos)
                            # Get chi angles
                            chi_angles = [pose.chi(j, seqpos) for j in range(1, res.nchi() + 1)]
                            cst_score = cat_scores.get(seqpos, {}).get('total', 0.0)
                            self.__flip_history.record(pi, seqpos, chi_angles, cst_score)

                        # Check stage gate - no flips before step N
                        if i < self.__flip_stage_gate:
                            print(f"  [Flip Gate] Step {i} < stage gate {self.__flip_stage_gate}, skipping flips")
                            continue

                        # Filter residues by gating criteria
                        eligible_residues = []
                        for seqpos in self.__catres_for_covalent:
                            res = pose.residue(seqpos)
                            res_name3 = res.name3()

                            # Check chi convergence
                            converged = self.__flip_history.is_chi_converged(pi, seqpos)
                            if not converged:
                                print(f"  [Flip Gate] {res_name3} {seqpos}: not converged, skipping")
                                continue

                            # Check CST persistence
                            persistent_poor_cst = self.__flip_history.is_cst_persistent(pi, seqpos)
                            if not persistent_poor_cst:
                                print(f"  [Flip Gate] {res_name3} {seqpos}: CST not persistently poor, skipping")
                                continue

                            # Check torsion_AB deviation gate (must be present and far off)
                            torsion_info = rosetta_utils.evaluate_torsion_ab_deviation(
                                pose,
                                seqpos,
                                ligand_seqpos=self.__ligand_seqpos,
                                deviation_threshold=self.__flip_torsion_ab_threshold
                            )
                            if not torsion_info['has_torsion_ab']:
                                print(f"  [Flip Gate] {res_name3} {seqpos}: no torsion_AB constraint, skipping")
                                continue
                            if not torsion_info['suggests_flip']:
                                deviation = torsion_info['deviation']
                                if deviation is None:
                                    deviation_str = "n/a"
                                else:
                                    deviation_str = f"{deviation:.1f}"
                                print(
                                    f"  [Flip Gate] {res_name3} {seqpos}: torsion_AB deviation {deviation_str} < {self.__flip_torsion_ab_threshold:.1f}, skipping"
                                )
                                continue

                            # Residue passes all gates
                            eligible_residues.append(seqpos)
                            print(f"  [Flip Gate] {res_name3} {seqpos}: ELIGIBLE (converged + persistent CST)")

                        # Attempt flips only for eligible residues
                        if eligible_residues:
                            flip_results = rosetta_utils.attempt_catres_sidechain_flips(
                                pose,
                                self.__scorefxn,
                                eligible_residues,  # Only pass residues that passed all gates
                                cst_threshold=self.__flip_cst_threshold,
                                do_minimize=True,
                                max_min_iter=self.__flip_max_min_iter,
                                ref_pose=self.__ref_pdb_pose,
                                ligand_seqpos=self.__ligand_seqpos,
                                torsion_ab_threshold=self.__flip_torsion_ab_threshold,
                                verbose=True
                            )
                            # Record flip results for aggregate statistics
                            self.__flip_tracker.record(flip_results, f"{command}_step{i}", pi)
                        else:
                            print(f"  [Flip Gate] No eligible residues for flip at step {i}")
        ...
        # Begin finalization stage if tracker is enabled
        if self.__tracker:
            self.__tracker.begin_stage("finalization", "Scoring final poses and computing final RMSD")
        ...
        for i, p in poses.items():
            ...
            # Track final catres RMSD for each pose
            if self.__tracker and self.__catres_for_covalent and self.__ref_pdb_pose:
                rmsd_result = rosetta_utils.calculate_catalytic_residue_rmsd(
                    p["pose"], self.__ref_pdb_pose, self.__catres_for_covalent,
                    atoms="functional", verbose=True
                )
                self.__tracker.log_catres_rmsd(rmsd_result, label=f"final_pose{i}")

                # Validate REMARK 666 consistency for final poses
                pdb_str = pyrosetta.distributed.io.to_pdbstring(p["pose"])
                if self.__original_remark666_lines:
                    remark_lines = self.__original_remark666_lines
                    print(f"[FastMPNN] Using original REMARK 666 lines for validation ({len(remark_lines)})")
                else:
                    remark_lines = [l for l in pdb_str.split("\n") if "REMARK 666" in l]
                validation = rosetta_utils.validate_remark666_consistency(
                    p["pose"], remark666_lines=remark_lines, verbose=True
                )
                if not validation['valid']:
                    print(f"[FastMPNN] WARNING: REMARK 666 validation failed for pose {i}!")
        ...
        # End finalization stage and print tracker summary
        if self.__tracker:
            self.__tracker.end_stage()
            print(self.__tracker.summary())
            # Save report if debug output dir is set
            if self.__tracker and self.__tracker.output_dir:
                self.__tracker.save_report(
                    os.path.join(self.__tracker.output_dir, f"{self.__name}_pipeline_report.json")
                )
