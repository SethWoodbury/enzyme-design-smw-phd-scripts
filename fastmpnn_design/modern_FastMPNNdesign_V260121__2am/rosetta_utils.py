...
def setup_pose_from_mpnn_output(mpnn_pdb_str, ref_pose, cst_io=None, sfx=None,
    ...
    Arguments:
        mpnn_pdb_str: PDB string from MPNN packed output
    ...
    # Write PDB string to temp file and load
    fd, tmp_path = tempfile.mkstemp(suffix='.pdb', prefix='mpnn_setup_')
    os.close(fd)

    try:
        with open(tmp_path, 'w') as f:
            f.write(mpnn_pdb_str)
    ...
    if verbose:
        print(f"  [SetupPose] Loaded MPNN pose: {pose.size()} residues")
