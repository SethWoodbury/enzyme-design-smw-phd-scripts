...
import time
import pandas as pd
proteinmpnn_dir = '/home/woodbuse/special_scripts/fastmpnn_design/fused_mpnn_api'
    ...
    def _thread_seq_to_pose(self, pose, sequence):
        ...
        for i, r in enumerate(sequence):
            if r not in "ACDEFKRYPGLIVMHNWQST":
                print(f"_thread_seq_to_pose: residue {i} aa1 {r} not a canonical amino acid?")
                continue
            if pose2.residue(i+1).name1() == r:
                continue
            mutres = pyrosetta.rosetta.protocols.simple_moves.MutateResidue()
            mutres.set_target(i+1)
            res_name = fusedmpnn.restype_1to3[r]
        ...
        def _figure_out_ch_resno(pose, resno):
            ...
            if isinstance(resno, str):
                if resno[0].isalpha():
                    ...
                    resno = int(resno)
                    chain_let = pose.pdb_info().chain(resno)
            resno = pose.pdb_info().number(resno)  # converting from pose-numbering to PDB-numbering for MPNN -- safe?
