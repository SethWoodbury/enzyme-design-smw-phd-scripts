...
class FastMPNNdesign():
    def __init__(self, model_type=None, N_seq=5, params=None, name=None,
        ...
        # IMPORTANT: HIS flips are now disabled by default because 180° rotation swaps
        # which nitrogen (Nd vs Ne) interacts with the ligand - this changes chemistry!
        self.__allow_his_flips = False  # Conservative default - don't flip HIS
    ...
    def set_allow_his_flips(self, allow: bool):
        """
        Set whether HIS flips are allowed.

        By default, HIS flips are DISABLED because a 180° rotation of the
        imidazole ring swaps which nitrogen (Nd vs Ne) interacts with the
        substrate, fundamentally changing the chemistry.

        Arguments:
            allow: True to allow HIS flips, False to disable (recommended)
        """
        self.__allow_his_flips = allow
        if allow:
            print("[FastMPNN] WARNING: HIS flips enabled - this can swap Nd/Ne interaction with substrate!")
    ...
    def apply(self, pose):
        ...
        for i, cmd in enumerate(self.script()):
            ...
            if command in ["repack", "min"]:
                ...
                # This helps escape local minima for symmetric sidechains (PHE, ASP, etc.)
                # NOTE: HIS flips are disabled by default (allow_his_flips=False) because
                # 180° rotation swaps Nd/Ne interaction with substrate - changes chemistry!
                if self.__enable_auto_flip and self.__catres_for_covalent:
                    for pi in poses:
                        flip_results = rosetta_utils.attempt_catres_sidechain_flips(
                            ...
                            allow_his_flips=self.__allow_his_flips,
            ...
            # Explicit flip_catres protocol operation
                ...
                if not self.__catres_for_covalent:
                    ...
                    print(f"[FastMPNN] flip_catres: threshold={threshold:.2f}, max_iter={max_iter}, allow_his={self.__allow_his_flips}")
                    for pi in poses:
                        flip_results = rosetta_utils.attempt_catres_sidechain_flips(
                            poses[pi]["pose"],
                            self.__scorefxn,
        ...
        if self.__tracker:
            ...
            if self.__debug_output_dir:
                self.__tracker.save_report(
                    os.path.join(self.__debug_output_dir, f"{self.__name}_pipeline_report.json")
                )
