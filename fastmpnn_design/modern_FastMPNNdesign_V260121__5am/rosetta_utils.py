...
# Residues where 180° flip changes functional chemistry (not just orientation)
# For these residues, we need additional checks before accepting a flip
# HIS: Swaps Nδ1/Nε2 - different nitrogens have different chemistry
# A flip that looks identical geometrically can completely change which N interacts with substrate
CHEMICALLY_SENSITIVE_FLIPS = {
    "HIS": {
        "functional_atoms": ["ND1", "NE2", "HD1", "HE2", "CE1", "CD2"],
        "max_functional_rmsd": 1.0,  # Maximum allowed RMSD of functional atoms (Angstroms)
    }
}


def _calculate_functional_atom_rmsd(pose_before, pose_after, seqpos, atom_names):
    """
    Calculate RMSD of specific functional atoms between two poses at a given residue.

    This is used to detect when a sidechain flip has significantly moved
    functional atoms (e.g., HIS nitrogens), which could change the chemistry
    even if the overall geometry looks similar.

    Arguments:
        pose_before: Pose before modification
        pose_after: Pose after modification
        seqpos: Residue sequence position
        atom_names: List of atom names to compare

    Returns:
        float: RMSD in Angstroms, or float('inf') if atoms not found
    """
    coords_before = []
    coords_after = []

    res_before = pose_before.residue(seqpos)
    res_after = pose_after.residue(seqpos)

    for atom_name in atom_names:
        if res_before.has(atom_name) and res_after.has(atom_name):
            coords_before.append(np.array(res_before.xyz(atom_name)))
            coords_after.append(np.array(res_after.xyz(atom_name)))

    if len(coords_before) < 2:
        return float('inf')

    coords_before = np.array(coords_before)
    coords_after = np.array(coords_after)
...
def attempt_catres_sidechain_flips(pose, sfx, catalytic_residues,
    ...
    IMPORTANT: For chemically sensitive residues like HIS, a 180-degree flip swaps
    which functional atoms (e.g., Nd vs Ne nitrogen) interact with the substrate.
    This can completely change the catalytic mechanism even if the geometry looks
    similar. By default, HIS flips are DISABLED to prevent this issue.
    ...
    Arguments:
        ...
        allow_his_flips: Allow HIS flips (default False - CONSERVATIVE)
        ref_pose: Reference pose for functional RMSD validation (optional but recommended)
    ...
    if verbose:
        print(f"\n  [Flip] Attempting sidechain flips for {len(cat_resnos)} catalytic residues (threshold: {cst_threshold:.2f} REU)")
    ...
    for seqpos in cat_resnos:
        ...
        # Check if this is a chemically sensitive residue that needs special handling
        is_chemically_sensitive = res_name3 in CHEMICALLY_SENSITIVE_FLIPS

        # Skip chemically sensitive residues unless explicitly allowed
        if is_chemically_sensitive and not allow_his_flips:
            if verbose:
                print(f"  [Flip] {res_name3} {seqpos}: SKIPPED - chemically sensitive residue")
                print(f"         (HIS flips swap Nd/Ne interaction - use allow_his_flips=True to override)")
            results[seqpos] = {
                'flipped': False,
                'improved': False,
                'reason': 'chemically_sensitive_disabled'
            }
            continue
        ...
        # For chemically sensitive residues, store pre-flip pose for RMSD comparison
        pre_flip_pose = None
        if is_chemically_sensitive:
            pre_flip_pose = pose.clone()
        ...
        # For chemically sensitive residues, check functional atom RMSD
        # This catches flips that look OK by constraint score but actually swap functional groups
        functional_rmsd = None
        if is_chemically_sensitive:
            config = CHEMICALLY_SENSITIVE_FLIPS[res_name3]
            # Compare against reference pose if available, otherwise use pre-flip state
            compare_pose = ref_pose if ref_pose is not None else pre_flip_pose
            functional_rmsd = _calculate_functional_atom_rmsd(
                compare_pose, pose, seqpos, config["functional_atoms"]
            )

            if verbose:
                print(f"  [Flip] {res_name3} {seqpos}: functional RMSD = {functional_rmsd:.2f} A (threshold: {config['max_functional_rmsd']:.1f} A)")

            # REJECT if functional RMSD exceeds threshold (atoms moved too much)
            if functional_rmsd > config["max_functional_rmsd"]:
                # Revert the flip
                ...
                if verbose:
                    print(f"  [Flip] {res_name3} {seqpos}: REJECTED - functional RMSD {functional_rmsd:.2f} > {config['max_functional_rmsd']:.1f} A")
                    print(f"         (flip would swap functional atoms - this is chemically dangerous)")

                results[seqpos] = {
                    'flipped': True,
                    'improved': False,
                    'reason': 'functional_rmsd_exceeded',
                    'functional_rmsd': functional_rmsd,
                    'old_cst': current_cst,
                    'new_cst': new_cst,
                    'reverted': True
                }
        ...
        # Decision: accept if constraint score improved
        ...
        if improved:
            flips_accepted += 1
            if verbose:
                print(f"  [Flip] {res_name3} {seqpos}: ACCEPTED - cst {current_cst:.2f} -> {new_cst:.2f}, score {original_score:.2f} -> {new_score:.2f}")
            results[seqpos] = {
                'flipped': True,
                'improved': True,
                'old_cst': current_cst,
