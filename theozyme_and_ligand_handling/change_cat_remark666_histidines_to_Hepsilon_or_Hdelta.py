...
"""
Purpose:
    Automatically enforce a specified protonation pattern on catalytic histidines
    annotated via “REMARK 666” in a PDB file.  For each HIS listed, the script will
    add or remove the δ-proton (HD1) or ε-proton (HE2) so that only the user-requested
    proton remains, and renumber all ATOM/HETATM records in a clean, logical order.

Inputs:
    --pdb <path>
        Path to the input PDB file containing “REMARK 666” annotations for HIS.
    --histidine_EorD_type_in_order_of_remark666_lines "<flags>"
        Space-separated sequence of “E” (ε) or “D” (δ) flags matching the order of
        HIS entries in REMARK 666.  Example: "E D E".
    --out <filename>
...
Logic:
    ...
    2. Scan for `REMARK 666 ... HIS chain resnum` lines in file order and collect
       (chain, resnum) pairs.
    3. Split the user’s E/D flag string and verify it matches the number of HIS found.
    4. Parse every ATOM/HETATM line into an AtomRecord object, preserving original text
       plus coordinate and atom-name metadata.
    5. For each catalytic HIS:
       a. Locate its ring atoms (ND1, NE2, CG, CE1, CD2) and any existing HD1/HE2.
       b. If both hydrogens are present, remove the undesired one.
       c. If the desired hydrogen is missing, compute its 3D position by:
          • Placing it on the bisector of the two adjacent ring bonds at ~1.0 Å
          • Rotating around the ring normal to achieve a 180° dihedral
       d. Insert the new hydrogen record immediately after the last HIS atom.
    6. Gather all surviving AtomRecords (excluding flagged removals), sort them by:
       (1) HETATM without “ORI ORI”, (2) ATOM, (3) HETATM with “ORI ORI”,
       then assign sequential serial numbers.
    7. Reconstruct the PDB: emit all non-ATOM/HETATM lines unchanged, interleave
       rewritten ATOM/HETATM lines in original file order, and write to the output file.

Outputs:
    A modified PDB file with:
      - Only the specified δ- or ε-proton on each catalytic HIS
      - All other atoms unchanged
      - Sequentially renumbered ATOM/HETATM records

Example:
    python change_cat_remark666_histidines_to_Hepsilon_or_Hdelta.py \\
        --pdb input_structure.pdb \\
        --histidine_EorD_type_in_order_of_remark666_lines "D E D" \\
        --out adjusted_structure.pdb

Key points:
  - Preserves original lines except for removing/adding HIS hydrogens.
  - Uses a ring-normal approach to place hydrogens with ~180° dihedral.
  - Renumbers lines in special order (HETATM w/o "ORI ORI", then ATOM, then HETATM w/ "ORI ORI").

Example Command:
python /home/woodbuse/special_scripts/theozyme_and_ligand_handling/change_cat_remark666_histidines_to_Hepsilon_or_Hdelta.py   --pdb group7_ASP_out_4muPA_dftTS_1_0_1_lig_SZC.pdb   --histidine_EorD_type_in_order_of_remark666_lines "D E D"   --out modified_structure.pdb
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--histidine_EorD_type_in_order_of_remark666_lines",
        required=True,
        help='String specifying which histidines are epsilon (E) or delta (D), e.g. "E D E".'
    )
    parser.add_argument(
...
def main():
    ...
    # 3) Parse E/D flags
    ED_list = args.histidine_EorD_type_in_order_of_remark666_lines.split()
    if len(histidines_in_remark666) != len(ED_list):
        print("ERROR: # of HIS in REMARK 666 != # of E/D flags.")
        sys.exit(1)
    ...
    # 5) For each HIS from REMARK 666
    for i, (chain, resnum) in enumerate(histidines_in_remark666):
        desired_type = ED_list[i]  # "E" or "D"
