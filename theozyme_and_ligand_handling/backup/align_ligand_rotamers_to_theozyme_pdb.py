...
Input Arguments:
  ...
  --output_suffix: Optional suffix for the output files before the .pdb extension
...
def process_files(original_pdb, rotamer_dir, output_dir, atom_pairs, ligands_to_remove_after_pair_fitting, output_suffix):
    """
    ...
    - output_suffix: Suffix for the output files before the .pdb extension
    """
    ...
    for rotamer_file in os.listdir(rotamer_dir):
        if rotamer_file.endswith(".pdb"):
            ...
            output_pdb = os.path.join(output_dir, f"{rotamer_name}{output_suffix}.pdb")
            print('')
            ...
            # Remove specified ligands after pair fitting
            if ligands_to_remove_after_pair_fitting:
                print(f"Removing specified ligands: {ligands_to_remove_after_pair_fitting}")
...
def main():
    ...
    parser.add_argument('--output_suffix', type=str, default='', help='Optional suffix for the output files before the .pdb extension')
    
    args = parser.parse_args()
    atom_pairs = parse_atom_pairs(args.atom_pairs)
    
    print("Starting ligand alignment process...")
    process_files(args.original_pdb, args.rotamer_dir, args.output_dir, atom_pairs, args.ligands_to_remove_after_pair_fitting, args.output_suffix)
