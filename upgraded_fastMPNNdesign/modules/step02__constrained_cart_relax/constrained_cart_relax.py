...
class ConstrainedCartesianRelax:
    ...
    def __init__(
        ...
        cart_bonded_weight: float = 3.0,
        mobile_radius: float = 10.0,
        fastrelax_repeats: int = 3,
        fastrelax_ramp_stages: int = 3,
        ...
        cart_bonded_max: float = 4.0,
        enable_bond_geometry_min: bool = False,
        ...
        # Score term weight adjustments (scale factors, 1.0 = default)
        fa_rep_scale: float = 0.5,
        fa_atr_scale: float = 1.0,
        fa_elec_scale: float = 1.0,
        # Ramping options for fa_rep
        ramp_fa_rep: bool = False,
        fa_rep_min_scale: float = 0.2,
    ):
        """
        ...
        # Score term weight adjustments
        self.fa_rep_scale = fa_rep_scale
        self.fa_atr_scale = fa_atr_scale
        self.fa_elec_scale = fa_elec_scale
        self.ramp_fa_rep = ramp_fa_rep
        self.fa_rep_min_scale = fa_rep_min_scale

        # Track current fa_rep scale for ramping
        self.current_fa_rep_scale = fa_rep_min_scale if ramp_fa_rep else fa_rep_scale
    ...
    def setup_scorefunction(self):
        ...
        # Store default weights for reference
        self.default_fa_rep = sfxn.get_weight(ScoreType.fa_rep)
        self.default_fa_atr = sfxn.get_weight(ScoreType.fa_atr)
        self.default_fa_elec = sfxn.get_weight(ScoreType.fa_elec)
        ...
        # Apply score term scale factors
        if self.fa_rep_scale != 1.0 or self.ramp_fa_rep:
            new_fa_rep = self.default_fa_rep * self.current_fa_rep_scale
            sfxn.set_weight(ScoreType.fa_rep, new_fa_rep)
            LOGGER.info(f"  fa_rep = {new_fa_rep:.4f} (default {self.default_fa_rep:.4f} × {self.current_fa_rep_scale:.2f})")

        if self.fa_atr_scale != 1.0:
            new_fa_atr = self.default_fa_atr * self.fa_atr_scale
            sfxn.set_weight(ScoreType.fa_atr, new_fa_atr)
            LOGGER.info(f"  fa_atr = {new_fa_atr:.4f} (default {self.default_fa_atr:.4f} × {self.fa_atr_scale:.2f})")

        if self.fa_elec_scale != 1.0:
            new_fa_elec = self.default_fa_elec * self.fa_elec_scale
            sfxn.set_weight(ScoreType.fa_elec, new_fa_elec)
            LOGGER.info(f"  fa_elec = {new_fa_elec:.4f} (default {self.default_fa_elec:.4f} × {self.fa_elec_scale:.2f})")
        ...
        if self.ramp_fa_rep:
            LOGGER.info(f"  fa_rep ramping: ENABLED (min={self.fa_rep_min_scale:.2f}x -> max={self.fa_rep_scale:.2f}x)")

        self.sfxn = sfxn
        return sfxn

    def update_fa_rep_weight(self, scale: float):
        """
        Update fa_rep weight during ramping.

        Args:
            scale: Scale factor to apply to default fa_rep weight
        """
        if self.sfxn is None:
            return

        self.current_fa_rep_scale = scale
        new_fa_rep = self.default_fa_rep * scale
        self.sfxn.set_weight(ScoreType.fa_rep, new_fa_rep)
        LOGGER.debug(f"  Updated fa_rep to {new_fa_rep:.4f} (scale={scale:.2f})")
    ...
    def run_adaptive_protocol(self, pose, sfxn, movemap, task_factory=None,
        ...
        if self.fa_rep_scale != 1.0 or self.fa_atr_scale != 1.0 or self.fa_elec_scale != 1.0:
            LOGGER.info(f"Score term scaling: fa_rep={self.fa_rep_scale}, fa_atr={self.fa_atr_scale}, fa_elec={self.fa_elec_scale}")
        if self.ramp_fa_rep:
            LOGGER.info(f"fa_rep ramping: {self.fa_rep_min_scale:.2f}x -> {self.fa_rep_scale:.2f}x over {self.max_adaptive_rounds} rounds")
        ...
        while True:
            ...
            # Update fa_rep weight if ramping is enabled
            if self.ramp_fa_rep:
                # Ramp from fa_rep_min_scale to fa_rep_scale across rounds
                progress = min(round_num / max(self.max_adaptive_rounds - 1, 1), 1.0)
                fa_rep_scale = self.fa_rep_min_scale + (self.fa_rep_scale - self.fa_rep_min_scale) * progress
                self.update_fa_rep_weight(fa_rep_scale)
                LOGGER.info(f"  fa_rep scale: {fa_rep_scale:.3f} (ramping {self.fa_rep_min_scale:.2f} -> {self.fa_rep_scale:.2f})")
...
def init_pyrosetta(params_files: List[str], nproc: int = 1, scorefunction: str = "ref2015_cart") -> None:
    ...
    # Note: beta_jan25_cart scorefunction itself has issues (unrecognized atom_type 'CS'),
    # but the -beta_jan25 init flag works and enables the corrections
    if "beta_jan25" in scorefunction:
        init_flags.append("-beta_jan25")
        LOGGER.info("  Enabling beta_jan25 corrections")
    elif "beta_nov16" in scorefunction:
        init_flags.append("-beta_nov16")
...
Examples:
  ...
  %(prog)s --test
    ...
    parser.add_argument("--cart_bonded_weight", type=float, default=3.0,
                        help="Weight for cart_bonded term (default: 3.0)")
    ...
    parser.add_argument("--fastrelax_ramp_stages", type=int, default=3,
                        help="Number of ramping stages per repeat (N, default: 3). "
                             "Total rounds = M × N (default: 3 × 3 = 9)")
    ...
    parser.add_argument("--cart_bonded_max", type=float, default=4.0,
                        help="Maximum cart_bonded weight cap (default: 4.0)")

    # Score term weight scaling
    parser.add_argument("--fa_rep_scale", type=float, default=0.5,
                        help="Scale factor for fa_rep term (default: 0.5). "
                             "Lower values allow more clashing, useful when constraints are hard to satisfy.")
    parser.add_argument("--fa_atr_scale", type=float, default=1.0,
                        help="Scale factor for fa_atr term (default: 1.0)")
    parser.add_argument("--fa_elec_scale", type=float, default=1.0,
                        help="Scale factor for fa_elec term (default: 1.0)")
    parser.add_argument("--ramp_fa_rep", action="store_true",
                        help="Ramp fa_rep from fa_rep_min_scale up to fa_rep_scale during adaptive protocol. "
                             "Similar to FastRelax ramping but across adaptive rounds.")
    parser.add_argument("--fa_rep_min_scale", type=float, default=0.2,
                        help="Starting fa_rep scale when ramping (default: 0.2). "
                             "Only used when --ramp_fa_rep is enabled.")

    # Bond geometry minimization
    parser.add_argument("--enable_bond_geometry_min", action="store_true", default=False,
                        help="Enable bondangle+bondlength minimization (default: False)")
    ...
    parser.add_argument("--scorefunction", type=str, default="ref2015_cart",
                        ...
                        help="Scorefunction to use (default: ref2015_cart). "
                             "ref2015_cart gave 34%% better bond geometry than beta in sweep testing. "
                             "Note: beta_jan25_cart has issues with atom type 'CS' and is not supported.")
    ...
    # Capture defaults for preset override detection
    args._defaults = parser.parse_args([])
    return args
...
def apply_preset(args):
    ...
    PRESETS = {
        ...
        "balanced": {
            # Optimized defaults from comprehensive hyperparameter sweep
            "fastrelax_repeats": 3,
            "fastrelax_ramp_stages": 3,
    ...
    if args.preset and args.preset in PRESETS:
        ...
        for key, value in preset.items():
            if not hasattr(args, key):
                continue
            # Only apply if user hasn't explicitly overridden this arg
            defaults = getattr(args, "_defaults", None)
            if defaults is not None and getattr(args, key) != getattr(defaults, key):
                continue
...
def main():
    ...
    relaxer = ConstrainedCartesianRelax(
        ...
        # Score term scaling
        fa_rep_scale=args.fa_rep_scale,
        fa_atr_scale=args.fa_atr_scale,
        fa_elec_scale=args.fa_elec_scale,
        ramp_fa_rep=args.ramp_fa_rep,
        fa_rep_min_scale=args.fa_rep_min_scale,
    )
