...
- All protocol presets (including custom breadth/depth modes)
...
# Protocol presets (basic)
PRESETS = ["fast", "balanced", "thorough", "design_only"]

# New custom presets (once implemented)
CUSTOM_PRESETS = ["breadth", "depth", "iterative_refine", "progressive", "geometry_first"]
...
def generate_preset_sweep(job_counter: int) -> Tuple[List[str], int]:
    """Generate sweep over presets."""
    commands = []

    for preset in PRESETS + CUSTOM_PRESETS:
        for rep in range(1, NUM_REPLICATES + 1):
            job_counter += 1
            out_name = f"job{job_counter:04d}__preset_{preset}__rep{rep}"
            out_dir = f"{OUTPUT_BASE}/{out_name}"

            cmd = make_base_cmd(out_dir, [f"--preset {preset}"])
            commands.append(cmd)
    ...
    for weight in GLOBAL_COORD_CST_WEIGHTS:
        if weight == 0.0:
            ...
            for rep in range(1, NUM_REPLICATES + 1):
                ...
                cmd = make_base_cmd(out_dir, ["--preset balanced"])
                commands.append(cmd)
        else:
...
def generate_custom_protocol_sweep(job_counter: int) -> Tuple[List[str], int]:
    ...
    for idx, protocol in enumerate(CUSTOM_PROTOCOLS, 1):
        for rep in range(1, NUM_REPLICATES + 1):
            job_counter += 1
            out_name = f"job{job_counter:04d}__custom_protocol_{idx}__rep{rep}"
            out_dir = f"{OUTPUT_BASE}/{out_name}"
...
def generate_combined_sweep(job_counter: int) -> Tuple[List[str], int]:
    ...
    breadth_configs = [
        ("breadth_high_temp", ["--mpnn_temperature 0.3", "--mpnn_num_designs 32"]),
        ("breadth_multi_round", ['--protocol "mpnn:T0.3:N16 -> torsional_relax:R1S2 -> mpnn:T0.2:N16"']),
    ]

    depth_configs = [
        ("depth_low_temp", ["--mpnn_temperature 0.05", "--mpnn_num_designs 8"]),
        ("depth_multi_relax", ['--protocol "mpnn:T0.1:N4 -> torsional_relax:R3S3 -> mpnn:T0.1:N4 -> torsional_relax:R3S3"']),
    ]

    for config_name, extra_args in breadth_configs + depth_configs:
        for rep in range(1, NUM_REPLICATES + 1):
            job_counter += 1
            out_name = f"job{job_counter:04d}__{config_name}__rep{rep}"
            out_dir = f"{OUTPUT_BASE}/{out_name}"

            cmd = make_base_cmd(out_dir, ["--preset balanced"] + extra_args)
            commands.append(cmd)
...
def generate_quick_subset() -> List[str]:
    ...
    # Just basic presets, 1 replicate
    for preset in ["fast", "balanced", "design_only"]:
        job_counter += 1
        out_name = f"job{job_counter:04d}__preset_{preset}__rep1"
        out_dir = f"{OUTPUT_BASE}/{out_name}"
        cmd = make_base_cmd(out_dir, [f"--preset {preset}"])
        commands.append(cmd)
    ...
    for temp in [0.1, 0.2]:
        ...
        out_dir = f"{OUTPUT_BASE}/{out_name}"
        cmd = make_base_cmd(out_dir, ["--preset balanced", f"--mpnn_temperature {temp}"])
        commands.append(cmd)
    ...
    # Custom protocols
    for idx, protocol in enumerate(CUSTOM_PROTOCOLS[:3], 1):
        job_counter += 1
        out_name = f"job{job_counter:04d}__custom_{idx}__rep1"
        out_dir = f"{OUTPUT_BASE}/{out_name}"
        cmd = make_base_cmd(out_dir, [f'--protocol "{protocol}"'])
        commands.append(cmd)
...
def generate_medium_subset() -> List[str]:
    ...
    # Presets with 2 replicates
    cmds, job_counter = generate_preset_sweep(job_counter)
    commands.extend(cmds[:len(PRESETS) * 2])  # 2 reps for basic presets
    ...
    for n_des in [4, 8, 16]:
        ...
        out_dir = f"{OUTPUT_BASE}/{out_name}"
        cmd = make_base_cmd(out_dir, ["--preset balanced", f"--mpnn_num_designs {n_des}"])
        commands.append(cmd)
    ...
    for rounds in [1, 3, 5]:
        ...
        out_dir = f"{OUTPUT_BASE}/{out_name}"
        cmd = make_base_cmd(out_dir, ["--preset balanced", f"--relax_rounds {rounds}"])
        commands.append(cmd)
    ...
    for weight in [1.0, 2.0, 4.0]:
        ...
        out_dir = f"{OUTPUT_BASE}/{out_name}"
        cmd = make_base_cmd(out_dir, ["--preset balanced", f"--cart_bonded_weight {weight}"])
        commands.append(cmd)
    ...
    for idx, protocol in enumerate(CUSTOM_PROTOCOLS, 1):
        for rep in [1, 2]:
            job_counter += 1
            out_name = f"job{job_counter:04d}__custom_{idx}__rep{rep}"
            out_dir = f"{OUTPUT_BASE}/{out_name}"
            cmd = make_base_cmd(out_dir, [f'--protocol "{protocol}"'])
            commands.append(cmd)
...
def main():
    ...
    # Summary
    print("\nSweep parameters:")
    print(f"  Presets: {PRESETS + CUSTOM_PRESETS}")
