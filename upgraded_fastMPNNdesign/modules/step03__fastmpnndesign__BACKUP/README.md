...
- **Modular protocol system** - Presets, custom strings, JSON/text protocol files
...
Based on hyperparameter sweep analysis:
...
|----------|---------|-------------|
| `--preset` | `balanced` | Protocol preset name |
| `--protocol` | - | Custom protocol string (overrides preset) |
| `--protocol_file` | - | Path to protocol JSON/text file (overrides preset) |
...
| Argument | Default | Description |
|----------|---------|-------------|
| `--conserve_favorable_interactions` | true | Enable H-bond keeper |
| `--no_conserve_interactions` | - | Disable interaction conservation |
| `--conservation_probability` | 0.5 | Probability to fix beneficial mutations |
| `--include_bb_hbond_constraints` | false | Include backbone H-bond constraints |

**Note:** Interaction conservation is **ENABLED by default**. Use `--no_conserve_interactions` to disable it. The `--conserve_favorable_interactions` flag is a legacy parameter that does nothing on its own (it's already true by default).

### Output Control

| Argument | Default | Description |
|----------|---------|-------------|
| `--num_final_designs` | 10 | Number of final designs to output |
...
| Argument | Default | Description |
|----------|---------|-------------|
| `--no-mpnn-server` | false | Disable MPNN server (use subprocess fallback) |
...
### Protocol String Syntax

Custom protocols are specified as steps separated by `->`:
...
- `target_count` - Target number of final designs (overrides `--num_final_designs`)
...
See `protocols/` directory for examples: `default.json`, `fast.json`, `thorough.json`, `progressive_expansion.json`.
...
├── protocol_parser.py              (91 KB)    Protocol parsing, presets, step types
