...
Sweeps over:
- Protocol presets (fast, balanced, thorough, aggressive)
- MPNN temperatures
...
PRESETS = ["fast", "balanced", "thorough", "aggressive", "design_only"]
...
for preset in PRESETS:
    job_id += 1
    out_name = f"job{job_id:03d}__preset_{preset}"
...
# 2. Temperature sweep (using balanced preset)
...
# 7. Combined sweeps (temperature x preset)
for preset in ["fast", "balanced"]:
    for temp in [0.1, 0.2]:
        job_id += 1
        out_name = f"job{job_id:03d}__{preset}__temp{temp}"
        out_dir = f"{OUTPUT_BASE}/{out_name}"
