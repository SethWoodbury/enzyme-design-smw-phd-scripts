...
class FastMPNNDesigner:
    ...
    def __init__(
        ...
        output_dir: str,
        preset: Optional[str] = None,  # Legacy/deprecated
        protocol: Optional[str] = None,  # Basename in protocols/ dir (default: "default")
        protocol_file: Optional[str] = None,  # Explicit file path
        # Design scope
        ...
        if not no_container and in_container():
            LOGGER.warning("Detected container environment. Consider using --no-container "
                          "if PyRosetta and MPNN are available in this container.")
            # Optionally auto-enable no_container mode:
        ...
        # Parse protocol
        # Priority: preset (legacy) > protocol_file (explicit path) > protocol (basename)
        if preset:
            # Legacy Python-defined presets - show big warning
            LOGGER.warning("")
            LOGGER.warning("=" * 70)
            LOGGER.warning("WARNING: --preset is DEPRECATED and uses legacy Python-defined protocols")
            LOGGER.warning("These presets are outdated and less efficient than JSON protocols.")
            LOGGER.warning("Please migrate to: --protocol <name>  (e.g., --protocol default)")
            LOGGER.warning("Available JSON protocols in protocols/ directory:")
            for f in sorted(Path(PROTOCOLS_DIR).glob("*.json")):
                LOGGER.warning(f"  - {f.stem}")
            LOGGER.warning("=" * 70)
            LOGGER.warning("")
            self.protocol_steps = ProtocolParser.parse(preset)
            self.protocol_str = f"preset:{preset}"
        elif protocol_file:
    ...
    def initialize(self) -> None:
        ...
        self.mpnn_runner = MPNNRunner(
            use_container=not self.no_container,
    ...
    def _run_mpnn_step(
        ...
        if self._mpnn_has_run and self.mpnn_num_designs_after_first is not None:
        ...
        for input_pdb in input_pdbs:
            ...
            mpnn_out = self.output_dir / f"mpnn_{time.time_ns()}"
        ...
        # Deduplicate sequences after MPNN
    ...
    def _dedupe_pdbs_by_sequence(self, pdbs: List[str]) -> List[str]:
        ...
        try:
            unique_seqs, unique_pdbs, _ = remove_duplicate_sequences(
                sequences, pdbs, metrics=None, keep_best_geometry=False
            )
            return unique_pdbs
        except Exception:
            return pdbs
    ...
    def _apply_scale_step(self, step: ScaleScoreTermStep) -> None:
        ...
        normalized = {normalize(k): float(v) for k, v in (step.terms or {}).items()}
        if not normalized:
            return
        if step.scope.lower() == "next":
    ...
    def _run_cluster_step(self, step: ClusterStep, pdbs: List[str]) -> None:
        ...
        if method == "sequence":
            # Farthest-first clustering by sequence identity
            ...
            if step.n_clusters:
                centers = [0]
                while len(centers) < step.n_clusters and len(centers) < len(pdbs):
                    best_idx = None
                    best_score = 1.0
            ...
            for i, pdb in enumerate(pdbs):
                if not centers:
                    assignments[pdb] = 0
                    continue
                # assign to closest center (highest identity)
                best = max(range(len(centers)), key=lambda k: identity(i, centers[k]))
                assignments[pdb] = best
    ...
    def _run_keep_cluster_best_step(self, step: KeepClusterBestStep, pdbs: List[str]) -> List[str]:
        ...
        if not pdbs or not self._cluster_assignments:
            return pdbs
    ...
    def _select_best_structures(
        ...
        def get_metrics(pdb: str) -> Dict[str, Any]:
            ...
            if "error" not in score:
                rosetta_score = score.get("total_score")
    ...
    def _run_final_diversify_step(
        ...
        1. CLI --num_final_designs (if explicitly provided)
        2. Step JSON target_count (if specified)
        3. Default: 10
        """
        # CLI overrides JSON if explicitly provided
        if self.num_final_designs is not None:
            target_count = self.num_final_designs
        elif step.target_count is not None:
    ...
    def _sort_pdbs_by_geometry(self, pdbs: List[str]) -> List[str]:
        ...
        for pdb in pdbs:
            try:
                ...
                geom = calc.calculate_bond_geometry()
                score = geom.get("bond_length_geometry", {}).get(
                    "unconstrained_only", {}).get("max", float('inf'))
    ...
    def _cross_temp_dedupe(
        ...
        for seq, candidates in seq_to_candidates.items():
            best_pdb, best_metrics, best_score = None, {}, float('inf')

            for pdb, temp in candidates:
                try:
                    calc = MetricsCalculator(
                        designed_pdb=pdb,
                        step02_pdb=self.step02_pdb,
                    ...
                    score = metrics.get("bond_geometry", {}).get(
                        "bond_length_geometry", {}).get("unconstrained_only", {}).get("max", float('inf'))

                    if score < best_score:
                        best_score, best_pdb, best_metrics = score, pdb, metrics
                except Exception:
                    if best_pdb is None:
                        best_pdb = pdb
    ...
    def _run_rosetta_relax(
        ...
        for input_pdb in input_pdbs:
            ...
            cst_json = str(cst_dir / f"{Path(input_pdb).stem}.cst.json")
            ...
            his_json = str(cst_dir / f"{Path(input_pdb).stem}.his_tautomer.json")
            his_map_for_json = {
    ...
    def _record_metrics_snapshot(
        ...
        snapshot = {
            ...
            "step_params": {k: (v.value if hasattr(v, "value") else v) for k, v in step.__dict__.items()},
            "structures": [],
        }
    ...
    def _finalize_designs(self, current_pdbs: List[str]) -> List[Dict]:
        ...
        for pdb in current_pdbs:
            ...
            restored_pdb = str(self.output_dir / f"{Path(pdb).stem}.final.pdb")
            try:
                full_mpnn_output_restoration(
                    mpnn_pdb=pdb,
                    ref_pdb=self.step02_pdb,
        ...
        scored.sort(key=lambda x: x[0])
        num_to_select = self.num_final_designs if self.num_final_designs is not None else 10
...
def main():
    ...
    parser.add_argument("--preset", default=None,
                       ...
                       help="[DEPRECATED/LEGACY] Python-defined protocol preset. "
                            "Use --protocol instead for JSON-based protocols.")
    ...
    # MPNN Server settings (persistent model for faster design)
    parser.add_argument("--no-mpnn-server", action="store_true",
                       help="Disable MPNN server (use subprocess for each MPNN call). "
                            "Server mode keeps model weights in GPU memory for ~5-10x speedup.")
    ...
    parser.add_argument("--no-auto-start-mpnn-server", action="store_true",
                       help="Don't auto-start MPNN server if not running")
    ...
    designer = FastMPNNDesigner(
        ...
        use_mpnn_server=not getattr(args, 'no_mpnn_server', False),
        mpnn_server_host=getattr(args, 'mpnn_server_host', 'localhost'),
        mpnn_server_port=getattr(args, 'mpnn_server_port', 5000),
        auto_start_mpnn_server=not getattr(args, 'no_auto_start_mpnn_server', False),
    )
