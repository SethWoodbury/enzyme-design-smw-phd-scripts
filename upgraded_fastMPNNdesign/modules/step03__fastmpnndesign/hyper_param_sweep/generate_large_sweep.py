...
def build_command(
    ...
    if protocol_file:
        cmd.append(f'--protocol_file "{protocol_file}"')
    elif protocol:
...
def protocol_file_for_string(protocol_str: str) -> str:
    """Write a protocol .txt file for a protocol string and return its path."""
    key = protocol_str.strip()
    if key in _PROTOCOL_FILE_CACHE:
        return str(_PROTOCOL_FILE_CACHE[key])

    import hashlib

    digest = hashlib.md5(key.encode("utf-8")).hexdigest()[:10]
    PROTOCOL_DIR.mkdir(parents=True, exist_ok=True)
    path = PROTOCOL_DIR / f"proto_{digest}.txt"

    if WRITE_PROTOCOL_FILES:
        # Always overwrite to keep content consistent
        path.write_text(key + "\n")
...
def generate_protocol_retest() -> List[Tuple[str, str]]:
    """Re-test built-in JSON protocols with fixed select_best."""
    jobs = []

    protocols = ["fast", "balanced", "thorough"]
    for protocol in protocols:
        jobs.append((f"protocol_{protocol}_fixed", protocol))
...
def main():
    ...
    for name, protocol in protocol_jobs:
        for rep in range(1, args.replicates + 1):
            ...
            protocol_file = protocol_file_for_string(protocol)
            cmd = build_command(job_name, protocol_file=protocol_file)
            all_jobs.append(cmd)
            job_num += 1
    ...
    for job_data in param_jobs:
        ...
        for rep in range(1, args.replicates + 1):
            job_name = f"job{job_num:04d}__{name}__rep{rep}"
            protocol_file = protocol_file_for_string(protocol) if protocol else None
            cmd = build_command(job_name, protocol_file=protocol_file, extra_args=extra_args)
            all_jobs.append(cmd)
            job_num += 1

    # Protocol re-test
    for name, protocol in generate_protocol_retest():
        for rep in range(1, args.replicates + 1):
            job_name = f"job{job_num:04d}__{name}__rep{rep}"
    ...
    print(f"  Protocol re-test: {len(generate_protocol_retest()) * args.replicates}")
