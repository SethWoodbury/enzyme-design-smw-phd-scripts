...
def test_json_protocol_parsing():
    ...
    json_content = {
        "steps": [
            {"type": "mpnn_multi", "strategies": [
                {"type": "design_core_shell", "temperature": 0.1, "num_designs": 2},
                {"type": "design_core_shell_flex", "temperature": 0.15, "num_designs": 2},
            ]},
    ...
    try:
        ...
        # Check first MPNN multi-step
        assert isinstance(steps[0], MPNNMultiStep), f"Expected MPNNMultiStep, got {type(steps[0])}"
        assert len(steps[0].strategies) == 2, f"Expected 2 strategies, got {len(steps[0].strategies)}"
        assert steps[0].strategies[0].temperature == 0.1
        assert steps[0].strategies[1].temperature == 0.15
...
def test_all_step_types_json():
    ...
    json_content = {
        "steps": [
            ...
            {"type": "mpnn_multi", "strategies": [
                {"type": "design_core_shell", "temperature": 0.15, "num_designs": 2},
            ...
            {"type": "minimize", "tolerance": 0.001, "max_iter": 500, "minimize_scope": "core_shell"},
            {"type": "repack", "repack_shell": 5.0},
            {"type": "select_best", "n": 2, "metric": "geometry"},
            {"type": "time_check", "max_elapsed": 120.0, "then": [
    ...
    try:
        ...
        assert isinstance(steps[1], MPNNMultiStep)
        assert len(steps[1].strategies) == 2
        assert isinstance(steps[2], MPNNStep) and steps[2].step_type == StepType.MPNN_PRIMARY
        assert isinstance(steps[3], MPNNStep) and steps[3].step_type == StepType.MPNN_SECONDARY
        assert steps[3].design_spheres == ["primary", "secondary"]
        assert isinstance(steps[4], MPNNStep) and steps[4].step_type == StepType.MPNN_2ND_SHELL

        # Check cart_relax with until_converged
        assert isinstance(steps[5], CartRelaxStep)
        assert steps[5].until_converged == True

        # Check torsional_relax
        assert isinstance(steps[6], TorsionalRelaxStep)

        # Check minimize
        assert isinstance(steps[7], MinimizeStep)
        assert steps[7].tolerance == 0.001
        assert steps[7].max_iter == 500
        assert steps[7].minimize_scope == "core_shell"

        # Check repack
        assert isinstance(steps[8], RepackStep)
        assert steps[8].repack_shell == 5.0

        # Check select_best
        assert isinstance(steps[9], SelectBestStep)
        assert steps[9].n == 2

        # Check time_check
        assert isinstance(steps[10], TimeCheckStep)
        assert steps[10].max_elapsed == 120.0
        assert len(steps[10].then_steps) == 3
        assert steps[9].metric == "geometry"
